...
Its completeness theorem is therefore the exact executable component needed
for a rational source-attached strict inert machine.
-/
...
theorem verifies_terminalGap
  ...
  have hcomponents := certificate.verifies_components hverify
  simpa only [gamma, Rat.cast_div, Rat.cast_ofNat] using
...
theorem verifies_no_uniformEquilibriumPayoff
  ...
  apply quittingGame_not_exists_uniformEquilibriumPayoff_of_terminalExploitabilityGap
    reward.realReward certificate.gamma_pos
  ...
  | true =>
      ...
      | some result =>
          ...
          | lower lowerRounds tree =>
              ...
              subst certificate
              exact ⟨hnormalized, rfl, rfl, hscale⟩
