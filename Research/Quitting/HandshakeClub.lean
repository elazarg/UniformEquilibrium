...
This module presents a quitting game as a partnership with exit clauses.
Every day each member privately chooses `Stay` (`false`) or `Walk`
...
`PeriodicAudit` below selects a positive-mass finite window and repeats its
rows.  `PeriodicAudit.rows_add_length` records the literal periodicity; no
strategic conclusion is attached to that construction in this module.
-/
...
/-- Repeating the selected window gives a root sequence with the selected
positive length as a literal period. -/
theorem rows_add_length
    (audit : PeriodicAudit club rotation) (time : ℕ) :
    audit.rows (time + audit.length) = audit.rows time := by
  unfold rows
