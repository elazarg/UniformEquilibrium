...
construction.  The playerwise contraction required by the production
periodic compiler is automatic from positive proper common hazard; it is not
an additional certificate premise.
-/
...
open CyclicKofNArithmetic CyclicKofNQuittingSchedule
  CyclicKofNPrimitiveBlocks CyclicKofNExactCollapseSchedule
...
theorem isUniformEquilibriumPayoff_of_exactCollapseNashBellmanCycle
  ...
  exact isUniformEquilibriumPayoff_of_cyclicNashBellmanCycle
    reward (exactCollapseBlock n k d) hA hpopulation
      β hβpos hβlt point initial hroot hedge

/-- The finite certificate has exactly `n = N/d` Bellman phases. -/
theorem exactCollapse_equilibriumCertificate_phaseCount
    {n k d : ℕ} [NeZero n] [NeZero d]
    (hkpos : 0 < k) (hkproper : k < n) :
