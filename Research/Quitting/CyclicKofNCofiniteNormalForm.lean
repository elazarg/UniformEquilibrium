...
theorem fourOfFive_period_eq_five
  ...
  exact fourOfFive_cyclicSchedule_period_eq_five A hA hG
...
/-- Concrete `4/5` finite compiler with one proper hazard per missing-player
phase.  The only game-specific assumption is the five-phase exact Nash
certificate. -/
theorem fourOfFive_isUniformEquilibriumPayoff_of_phaseHazardCertificate
  ...
  exact isUniformEquilibriumPayoff_of_finitePhaseHazardNashCertificate
    reward A hAnonempty (by omega) β hβpos hβlt hnash initial

/-- In constant-hazard `4/5`, the full-game certificate is exactly the
endpoint-layer certificate with `K = 4`, hence with a possible degree-five
outside-entrant term. -/
theorem fourOfFive_fullCertificate_iff_endpointLayers
    (reward : {S : Finset G // S.Nonempty} → Payoff G)
...
/-- Under the literal four-movers-only rule, the corresponding endpoint
certificate uses degree-four layers only. -/
theorem fourOfFive_restrictedCertificate_iff_degreeFourLayers
    (reward : {S : Finset G // S.Nonempty} → Payoff G)
    (A : Finset G) (hA : A.card = 4)
    (β : ℝ) (hβ0 : 0 ≤ β) (hβ1 : β ≤ 1) :
