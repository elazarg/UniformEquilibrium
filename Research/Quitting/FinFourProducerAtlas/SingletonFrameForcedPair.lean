...
/-!
# Forced pairs from a neutral singleton frame

This module factors the local forced-pair construction through one neutral
frame carrying its resolution explicitly. Source-preserving and source-faithful
frames can both delegate to it without identifying their source chronologies.
...
structure FinFourSingletonFrame
...
namespace FinFourSingletonFrame
...
theorem pureSingleton_postDateSpine_eq_reference
  ...
  calc
    quittingAllContinueProfileSpine reward frame.pureSingletonProfile
      ...
      exact congrFun
        (frame.pureSingletonProfile_at_of_ne time (by omega) player) history
    _ = _ := frame.postDateSpine_eq_reference

end FinFourSingletonFrame

/-- A full-gap forced pair and one exact paid endpoint at the frame's explicit
resolution. -/
structure FinFourSingletonFrameForcedPairPacket
    (frame : FinFourSingletonFrame source) where
...
namespace FinFourSingletonFrameForcedPairPacket

variable {frame : FinFourSingletonFrame source}

/-- The forced terminal is the displayed pair. -/
theorem forcedTerminal_val
    (packet : FinFourSingletonFrameForcedPairPacket frame) :
...
def payerGain
    (packet : FinFourSingletonFrameForcedPairPacket frame) : ℝ :=
...
theorem forcedPair_postDateSpine_eq_reference
  ...
  calc
    quittingAllContinueProfileSpine reward packet.forcedAdapter.targetProfile
      ...
      intro player time history htime
      exact congrFun
        (packet.forcedPairProfile_at_of_ne time (by omega) player) history
    _ = _ := frame.postDateSpine_eq_reference
...
theorem payerTarget_postDateSpine_eq_reference
  ...
  calc
    quittingAllContinueProfileSpine reward packet.payerAdapter.targetProfile
      ...
      intro player time history htime
      exact congrFun
        (packet.payerTargetProfile_at_of_ne time (by omega) player) history
    _ = _ := frame.postDateSpine_eq_reference

end FinFourSingletonFrameForcedPairPacket
...
theorem nonempty_forcedPairPacket
    (frame : FinFourSingletonFrame source) :
    Nonempty (FinFourSingletonFrameForcedPairPacket frame) := by
