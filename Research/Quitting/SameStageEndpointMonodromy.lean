...
This Research module develops finite same-stage endpoint iteration from the
literal one-date profile interface.  The checked collision transfer uses
`quittingStagePureEndpointBehaviorDeviation`, which canonicalizes off-path
behavior.
