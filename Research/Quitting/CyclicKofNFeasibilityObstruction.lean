...
guarantees one.  Therefore no proper positive-hazard translated-block word is
exact Nash for this reward.
-/
...
open CyclicKofNClosedPhaseHazards
open CyclicKofNPhaseHazards
open CyclicKofNResetRetentionGeneralNoGo
...
translated block and every proper common hazard, the owner-membership game
fails the residual finite cyclic Nash certificate. -/
theorem not_finiteCyclicNashCertificate_selfMembershipReward
    (owner : G) (A : Finset G) (hA : A.Nonempty)
    (hproper : A ≠ Finset.univ)
    (β : ℝ) (hβpos : 0 < β) (hβlt : β < 1) :
    ¬ IsFiniteCyclicNashCertificate
      selfMembershipReward A β hβpos.le hβlt.le := by
  intro hnash
  ...
  have htail : tail owner ≤ 1 :=
    (le_abs_self (tail owner)).trans htailAbs
  exact not_isZeroNash_selfMembershipReward_of_pureContinue
    owner tail (cyclicPhaseRoots A β hβpos.le hβlt.le phase)
...
theorem not_finiteCyclicNashCertificate_selfMembershipReward_of_card_lt
  ...
  exact not_finiteCyclicNashCertificate_selfMembershipReward
    owner A hA hproper β hβpos hβlt
...
theorem not_finitePhaseHazardNashCertificate_selfMembershipReward
  ...
  have hcontracts :
    ...
    exact prod_one_sub_lt_one_of_mem
      (((cyclicSchedule A).active phase.val).erase owner)
      (fun _player => β phase)
      (fun _ _ => (hβpos phase).le)
      (fun _ _ => hβ1 phase)
      opponent hopponent (hβpos phase)
  have hpure :
      cyclicPhaseHazardRoots A β
          (fun p => (hβpos p).le)
...
theorem not_forall_reward_finiteCyclicNashCertificate
    ...
    ¬ ∀ reward : {S : Finset G // S.Nonempty} → Payoff G,
      IsFiniteCyclicNashCertificate reward A β hβpos.le hβlt.le := by
  intro hproducer
  exact not_finiteCyclicNashCertificate_selfMembershipReward
    owner A hA hproper β hβpos hβlt (hproducer selfMembershipReward)
...
theorem not_forall_reward_finitePhaseHazardNashCertificate
  ...
  exact not_finitePhaseHazardNashCertificate_selfMembershipReward
    owner A hA hproper β hβpos hβ1 (hproducer selfMembershipReward)
...
theorem fourOfFive_not_finiteCyclicNashCertificate_selfMembershipReward
    (owner : G) (A : Finset G) (hA : A.card = 4)
    (hG : Fintype.card G = 5)
    (β : ℝ) (hβpos : 0 < β) (hβlt : β < 1) :
    ¬ IsFiniteCyclicNashCertificate
      selfMembershipReward A β hβpos.le hβlt.le := by
  ...
  exact not_finiteCyclicNashCertificate_selfMembershipReward_of_card_lt
    owner A hnonempty hcard β hβpos hβlt
...
/-- The closed-boundary compiler consumes the saturated certificate. -/
theorem selfMembershipReward_fullSupportCyclicUniformPayoff
    (witness : G) (hG : 1 < Fintype.card G)
  ...
  exact isUniformEquilibriumPayoff_of_finitePhaseHazardNashCertificate_closed
    selfMembershipReward (Finset.univ : Finset G)
      ⟨witness, Finset.mem_univ _⟩ hG
      (allSurePhaseHazard Finset.univ)
