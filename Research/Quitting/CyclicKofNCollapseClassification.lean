...
/-- Construct a witness from a positive common divisor. -/
...
`d` is attainable iff it is a positive common divisor of `K` and `N`. -/
theorem isCyclicCollapseFactor_iff
  ...
  constructor
  · intro h
    exact ⟨h.positive, h.dvd_active, h.dvd_population⟩
  · rintro ⟨hdpos, hdK, hdN⟩
...
theorem isCyclicCollapseFactor_iff_dvd_gcd
  ...
  constructor
  · rintro ⟨hdpos, hdK, hdN⟩
    exact ⟨hdpos, Nat.dvd_gcd hdK hdN⟩
  · rintro ⟨hdpos, hdgcd⟩
    exact ⟨hdpos, hdgcd.trans (Nat.gcd_dvd_left K N),
...
theorem exists_collapseWitness_period_eq_reducedPopulation
  ...
  have hgcdpos : 0 < K.gcd N := Nat.gcd_pos_of_pos_left N hKpos
  have hattain : IsCyclicCollapseFactor K N (K.gcd N) :=
    isCyclicCollapseFactor_of_commonDivisor hKpos hKN hgcdpos
