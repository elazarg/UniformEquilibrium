...
The formal dynamical interpretation is the standard chattering word: use microscopic
...
exactly `floor(N * mass i) / N`.  Thus any nonlinear reset system with a
uniform second-order composition estimate would turn a tangent charged
circulation into a semantic reset loop with fixed abstract charge and
vanishing endpoint seam.

This file proves the rounding algebra only.  Two quitting-game inputs remain:
a uniform local composition estimate for the executable stopping-law reset
maps, including control of best-response witness switches, and a compiler
turning abstract reset charge into state-matched chronological absorption (or
into the complementary harmonic-Never contradiction).  Reset debt gain is
not itself game-time absorption.
-/
...
variable {State Move : Type*} [Fintype State] [Fintype Move]
...
theorem chatteringCoefficient_le
  ...
  have hfloor : (⌊(N : ℝ) * mass move⌋₊ : ℝ) ≤
      (N : ℝ) * mass move :=
    Nat.floor_le (mul_nonneg hNreal.le (hmass move))
...
theorem abs_chatteringCoefficient_sub_mass_le
  ...
  have hle := chatteringCoefficient_le mass N hN hmass move
...
theorem abs_sum_chatteringCoefficient_mul_column_le
    ...
    (hbalance : ∀ state, ∑ move, mass move * column move state = 0)
    (state : State) :
  ...
  have heq : (∑ move,
    ...
    calc
      (∑ move, chatteringCoefficient mass N move * column move state) =
        ...
        ring
      _ = _ := by rw [hbalance state, add_zero]
  rw [heq]
  calc
    |∑ move,
        (chatteringCoefficient mass N move - mass move) *
    ...
    _ ≤ ∑ move, (1 / N) * |column move state| := by
      ...
      exact mul_le_mul_of_nonneg_right
        (abs_chatteringCoefficient_sub_mass_le mass N hN hmass move)
        (abs_nonneg _)
  ...
  · intro state
    simpa only [← chatteringCoefficient_eq_count_div] using
      abs_sum_chatteringCoefficient_mul_column_le
        column mass N hN hmass hbalance state
  · simpa only [← chatteringCoefficient_eq_count_div] using
...
most `N` of completed microcycles and any partial next cycle, every
linearized coordinate stays within `O(1/N)` of the base.  No ordering of the
moves inside a microcycle is required. -/
theorem HasNormalizedPositiveChargedCirculation.abs_chatteringPrefix_le
    (column : Move → State → ℝ) (charge : Move → ℝ)
    (C : HasNormalizedPositiveChargedCirculation column charge)
    (N completed : ℕ) (hN : 0 < N) (hcompleted : completed ≤ N)
    (partialCount : Move → ℕ)
    (hpartial : ∀ move, partialCount move ≤ chatteringCount C.choose N move)
    (state : State) :
    |∑ move,
        chatteringPrefixCoefficient C.choose N completed partialCount move *
          column move state| ≤
      ((∑ move, |column move state|) +
        ∑ move, C.choose move * |column move state|) / N := by
  let mass : Move → ℝ := C.choose
  have hmass : ∀ move, 0 ≤ mass move := C.choose_spec.1
  have hbalance : ∀ state,
      ∑ move, mass move * column move state = 0 := C.choose_spec.2.1
  change |∑ move,
