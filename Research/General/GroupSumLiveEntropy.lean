...
* `bind_uniform_mul_eq_of_kernel`: the deviator's law may itself be the
  ...
  arbitrary kernel `k : S → PMF G`; this is still covered by
  ...
  `bind_mul_uniform_eq` are already the strongest *one-shot, simultaneous*
...
* **One round only.** No claim is made about repeated or adaptively linked
  rounds; `lastMover_forces` already shows that letting the deviator observe
...
namespace GroupSumLiveEntropy

variable {G : Type} [Fintype G] [Group G] [Nonempty G]
