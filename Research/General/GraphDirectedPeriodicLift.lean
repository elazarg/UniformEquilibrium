...
/-!
# Periodic symbolic paths have periodic geometric lifts
...
same contraction estimate over that common prefix.  The terminal vertex
equality is recorded explicitly so the zero-length case is also meaningful. -/
...
/-- Quantitative continuity of the coding map: a common edge prefix gives
exponentially close current values. -/
