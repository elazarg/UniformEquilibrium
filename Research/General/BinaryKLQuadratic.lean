...
machine-checks that scalar fact, independent of any detector, game, or
process construction.

We define binary KL divergence directly,

`klBin q p = q * log (q / p) + (1 - q) * log ((1 - q) / (1 - p))`,

without going through Mathlib's measure-theoretic `klDiv`, and prove three
facts on the open square `p, q ∈ (0, 1)`:

* `klBin_nonneg` : Gibbs' inequality, `0 ≤ klBin q p`.
* `two_mul_sq_le_klBin` : the binary Pinsker inequality with the sharp
  constant, `2 * (q - p) ^ 2 ≤ klBin q p`.
* `klBin_le_sq_div` : the chi-squared upper bound,
  `klBin q p ≤ (q - p) ^ 2 / (p * (1 - p))`.

Together these sandwich `klBin` between two quadratics of `(q - p)` whose
ratio is bounded on any compact sub-square, which is the precise sense in
which "detection information is `Θ(δ²)`". The symmetric-point corollaries
`le_klBin_half` and `klBin_half_le` record the clean numerical instance used
in the survey write-up: for `1/2 + δ ∈ (0, 1)`,

`2 * δ ^ 2 ≤ klBin (1/2 + δ) (1/2) ≤ 4 * δ ^ 2`.

## What this file does NOT claim

* **No process-level statement.** This is a single-observation (one-shot,
...
/-- Binary Kullback-Leibler divergence `KL(Bernoulli q ‖ Bernoulli p)`, defined
directly by its closed form rather than through Mathlib's measure-theoretic
`klDiv`. Meaningful for `p, q ∈ (0, 1)`. -/
noncomputable def klBin (q p : ℝ) : ℝ :=
  q * Real.log (q / p) + (1 - q) * Real.log ((1 - q) / (1 - p))

/-- For `a, b > 0`, `Real.log (a / b) ≥ 1 - b / a`. This is `Real.log x ≤ x - 1`
applied to the reciprocal ratio `b / a`, after converting `log (a / b)` to
`-log (b / a)`. It is the one-line engine behind Gibbs' inequality below. -/
private theorem one_sub_div_le_log_div {a b : ℝ} (ha : 0 < a) (hb : 0 < b) :
...
/-- The chi-squared identity behind the upper bound: `q²/p + (1-q)²/(1-p) - 1`
equals `(q - p)² / (p * (1 - p))`. -/
...
/-- Chi-squared upper bound: `klBin q p ≤ (q - p)² / (p (1 - p))`, from the
termwise bound `log x ≤ x - 1` applied to `q / p` and `(1 - q) / (1 - p)`. -/
...
/-- The binary Pinsker inequality with the sharp constant `2`:
`2 * (q - p)² ≤ klBin q p`. Proved by fixing `p`, setting
`F x = klBin x p - 2 * (x - p)²`, and showing `F` attains its minimum `0` at
`x = p` via a second-derivative sign argument: `F'' x = 1/x + 1/(1-x) - 4 ≥ 0`
on `(0,1)` because `x * (1 - x) ≤ 1/4` (AM-GM), so `F'` is monotone with
`F' p = 0`, so `F' ≤ 0` before `p` and `F' ≥ 0` after `p`, so `F` decreases
then increases around `p`. -/
...
/-- Upper half of the symmetric-point corollary: at `p = 1/2`, `klBin` is at
most `4 δ²` for a deviation `δ`. -/
...
/-- Lower half of the symmetric-point corollary: at `p = 1/2`, `klBin` is at
least `2 δ²` for a deviation `δ`. -/
