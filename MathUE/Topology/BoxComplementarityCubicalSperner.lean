...
This module labels a rational cubical grid by the first coordinate at which
the projected complementarity direction points down, with the upper boundary
included as in the reduced-label convention of the pinned cubical Sperner
theorem.

The result here is global and finite: the number of complete grid simplices is
odd at every positive resolution. Local counts, mesh stability, and local
indices are established by the subsequent signed-degree layers.
-/
