...
/-!
# Directed multigraphs and finite walks

This file supplies the graph-theoretic carrier shared by the transport and
bounded-discrepancy developments.  A directed multigraph has explicit edge
identities, so parallel edges remain distinct.  Its finite walks are indexed by
their endpoints and retain their chronological edge lists.

The module contains only the reusable finite-walk calculus: lengths and edge
lists, endpoint facts, concatenation, one-edge walks, edge multiplicities, and
splitting and splicing at visited edges or vertices.  Edge labels, transport,
charges, discrepancy, circulations, and Euler realization belong to separate
modules built on this one.
-/
