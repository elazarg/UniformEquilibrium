...
theorem exists_twoSupported_simplex_preserving_first_improving_second
    ...
    ∃ target : Convexity.StdSimplex ℝ Atom,
      Fintype.card {atom // target.weights atom ≠ 0} ≤ 2 ∧
  ...
  have hfeasible_iff (weight : Atom → ℝ) :
      weight ∈ standardFeasibleSet matrix rhs ↔
        weight ∈ simplexWeights Atom ∧ ∑ atom, weight atom * first atom =
          wsum source first := by
    simp only [standardFeasibleSet, Set.mem_ofPred_eq, funext_iff, Fin.forall_fin_two,
      matrix, rhs, Matrix.mulVec, dotProduct, Matrix.cons_val_zero,
      Matrix.cons_val_one, mul_comm, mul_one,
      mem_simplexWeights]
    tauto
  have hsource : (source.weights : Atom → ℝ) ∈ standardFeasibleSet matrix rhs := by
    exact (hfeasible_iff (source.weights : Atom → ℝ)).mpr
      ⟨mem_simplexWeights.mpr ⟨source.weights_nonneg, source.total_of_fintype⟩, rfl⟩
  have hcompact : IsCompact (standardFeasibleSet matrix rhs) := by
    exact (isCompact_simplexWeights Atom).of_isClosed_subset
  ...
  obtain ⟨optimal, hoptimalFeasible, hoptimal⟩ := hcompact.exists_isMaxOn
    ⟨(source.weights : Atom → ℝ), hsource⟩
    (finiteDotContinuousLinearMap second).continuous.continuousOn
  have hstandard : IsStandardOptimal matrix rhs second optimal := by
    exact ⟨hoptimalFeasible, fun weight hweight => by
      simpa only [finiteDotContinuousLinearMap_apply, Set.mem_ofPred_eq] using
        hoptimal hweight⟩
  ...
  refine ⟨⟨Finsupp.equivFunOnFinite.symm sparse,
    (mem_simplexWeights.mp hsparse.1).1, ?_⟩, ?_, hsparse.2, ?_⟩
  · rw [Finsupp.equivFunOnFinite_symm_sum]
    exact (mem_simplexWeights.mp hsparse.1).2
  · have hcard :=
      (linearIndependent_supportColumns_of_extreme_standardFeasible
        matrix rhs hsparseExtreme).fintype_card_le_finrank
    simpa only [Module.finrank_fintype_fun_eq_card, Fintype.card_fin,
      Finsupp.equivFunOnFinite_symm_apply_apply] using hcard
  · change (∑ atom, source.weights atom * second atom) ≤
      ∑ atom, sparse atom * second atom
    simpa only [mul_comm] using
      hsparseOptimal.2 (source.weights : Atom → ℝ) hsource
...
theorem exists_twoSupported_simplex_maximizer
    ...
    (source : Convexity.StdSimplex ℝ Atom)
    (hsource : ∀ candidate : Convexity.StdSimplex ℝ Atom,
    ...
    ∃ target : Convexity.StdSimplex ℝ Atom,
      ...
      objective (wsum target first) (wsum target second) =
        objective (wsum source first) (wsum source second) ∧
      ∀ candidate : Convexity.StdSimplex ℝ Atom,
        objective (wsum candidate first) (wsum candidate second) ≤
...
def threeMarginalValue (kernel : First → Second → Third → ℝ)
    (first : Convexity.StdSimplex ℝ First) (second : Convexity.StdSimplex ℝ Second)
    (third : Convexity.StdSimplex ℝ Third) : ℝ :=
...
theorem threeMarginalValue_swap_first_second (kernel : First → Second → Third → ℝ)
    (first : Convexity.StdSimplex ℝ First) (second : Convexity.StdSimplex ℝ Second)
    (third : Convexity.StdSimplex ℝ Third) :
...
theorem exists_three_twoSupported_marginals_preserving_first_improving_second
    ...
    (first : Convexity.StdSimplex ℝ First) (second : Convexity.StdSimplex ℝ Second)
    (third : Convexity.StdSimplex ℝ Third) :
    ∃ nextFirst : Convexity.StdSimplex ℝ First, ∃ nextSecond : Convexity.StdSimplex ℝ Second,
      ∃ nextThird : Convexity.StdSimplex ℝ Third,
        Fintype.card {i // nextFirst.weights i ≠ 0} ≤ 2 ∧
        Fintype.card {j // nextSecond.weights j ≠ 0} ≤ 2 ∧
        Fintype.card {k // nextThird.weights k ≠ 0} ≤ 2 ∧
...
theorem continuous_threeMarginalValue (kernel : First → Second → Third → ℝ) :
    Continuous (fun source :
      Convexity.StdSimplex ℝ First ×
        Convexity.StdSimplex ℝ Second × Convexity.StdSimplex ℝ Third =>
      threeMarginalValue kernel source.1 source.2.1 source.2.2) := by
  unfold threeMarginalValue wsum dotProduct
  apply continuous_finsetSum
  intro i _
  apply Continuous.mul
  · exact (Convexity.StdSimplex.continuous_coord i).comp continuous_fst
  · apply continuous_finsetSum
    intro j _
    apply Continuous.mul
    · exact (Convexity.StdSimplex.continuous_coord j).comp (continuous_fst.comp continuous_snd)
    · apply continuous_finsetSum
      intro k _
      exact ((Convexity.StdSimplex.continuous_coord k).comp
...
theorem exists_three_twoSupported_marginals_maximizer
    ...
    ∃ first : Convexity.StdSimplex ℝ First, ∃ second : Convexity.StdSimplex ℝ Second,
      ∃ third : Convexity.StdSimplex ℝ Third,
        Fintype.card {i // first.weights i ≠ 0} ≤ 2 ∧
        Fintype.card {j // second.weights j ≠ 0} ≤ 2 ∧
        Fintype.card {k // third.weights k ≠ 0} ≤ 2 ∧
        ∀ otherFirst : Convexity.StdSimplex ℝ First, ∀ otherSecond : Convexity.StdSimplex ℝ Second,
          ∀ otherThird : Convexity.StdSimplex ℝ Third,
  ...
  let value := fun source :
      Convexity.StdSimplex ℝ First × Convexity.StdSimplex ℝ Second × Convexity.StdSimplex ℝ Third =>
