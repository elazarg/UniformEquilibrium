...
/-!
# Reversible Dirichlet contraction
...
share an invariant law `π` and a common spectral gap `γ`.

Setup: a finite state space `S`, a nonnegative weight `π`, and a
...
and it equals `⟨f,f⟩_π - ⟨f,Pf⟩_π`.  A spectral-gap hypothesis
`γ * ‖f‖²_π ≤ E_P(f,f)` for mean-zero `f` then controls `⟨f,Pf⟩_π`.  For a
...
Poisson-type bound on `⟨f,Pf⟩_π` to a genuine norm contraction
...
spectral gap alone bounds `⟨f,Pf⟩_π` but does not control the negative part
...
address controlled kernels that fail to share one invariant law or
reversibility.
-/

namespace Math.Probability.ReversibleDirichletContraction

open scoped BigOperators

variable {S : Type} [Fintype S] [DecidableEq S]

/-- The `π`-weighted inner product `⟨f,g⟩_π = Σ_x π x * f x * g x`. -/
def wip (π : S → ℝ) (f g : S → ℝ) : ℝ := ∑ x, π x * f x * g x

/-- The `π`-weighted squared norm `‖f‖²_π = ⟨f,f⟩_π`. -/
...
omit [DecidableEq S] in
/-- The reversible Dirichlet identity: `E_P(f,f) = ‖f‖²_π - ⟨f,Pf⟩_π`. -/
...
theorem wnormSq_act_le (π : S → ℝ) (Q : S → S → ℝ) (hπ : ∀ x, 0 ≤ π x)
  ...
  have step1 : wnormSq π (act Q f) ≤ ∑ x, ∑ y, π x * Q x y * (f y) ^ 2 := by
    ...
    calc π x * (∑ y, Q x y * f y) ^ 2 ≤ π x * ∑ y, Q x y * (f y) ^ 2 :=
          mul_le_mul_of_nonneg_left hjensen (hπ x)
...
/-- A lazy mixture with a row-stochastic auxiliary kernel is row-stochastic. -/
theorem row_sum_of_lazy (P Q : S → S → ℝ)
    (hQ1 : ∀ x, ∑ y, Q x y = 1)
    (hPQ : ∀ x y, P x y = (1 / 2) * (if x = y then 1 else 0) + (1 / 2) * Q x y)
    (x : S) : ∑ y, P x y = 1 := by
  simp_rw [hPQ x]
  rw [Finset.sum_add_distrib, ← Finset.mul_sum, ← Finset.mul_sum, hQ1 x]
  norm_num

omit [Fintype S] in
/-- A lazy mixture inherits reversibility from its auxiliary kernel. -/
theorem reversible_of_lazy (π : S → ℝ) (P Q : S → S → ℝ)
    (hQrev : ∀ x y, π x * Q x y = π y * Q y x)
    (hPQ : ∀ x y, P x y = (1 / 2) * (if x = y then 1 else 0) + (1 / 2) * Q x y)
    (x y : S) : π x * P x y = π y * P y x := by
  by_cases hxy : x = y
  · subst y
    rfl
  · rw [hPQ x y, hPQ y x]
    ...
    calc
      ...
      _ = π y * (1 / 2 * Q y x) := by ring

/-- Laziness upgrades the Poisson-type bound `⟨f,Pf⟩_π` into a genuine norm
bound: `‖Pf‖²_π ≤ ⟨f,Pf⟩_π`.  Algebraically, with `P = (1/2)(I+Q)`,
`4(⟨f,Pf⟩_π - ‖Pf‖²_π) = ‖f‖²_π - ‖Qf‖²_π ≥ 0` by `wnormSq_act_le`. -/
theorem wnormSq_act_le_wip_act_of_lazy
    (π : S → ℝ) (P Q : S → S → ℝ) (hπ : ∀ x, 0 ≤ π x)
    (hQ0 : ∀ x y, 0 ≤ Q x y) (hQ1 : ∀ x, ∑ y, Q x y = 1)
    (hQrev : ∀ x y, π x * Q x y = π y * Q y x)
    (hPQ : ∀ x y, P x y = (1 / 2) * (if x = y then 1 else 0) + (1 / 2) * Q x y)
    (f : S → ℝ) :
...
theorem wnormSq_act_le_of_lazy_of_gap (π : S → ℝ) (P : S → S → ℝ) (γ : ℝ)
  ...
  have hP1 : ∀ x, ∑ y, P x y = 1 := row_sum_of_lazy P Q hQ1 hPQ
  have hrev : ∀ x y, π x * P x y = π y * P y x :=
    reversible_of_lazy π P Q hQrev hPQ
...
they share a common spectral gap `γ`, then every mean-zero mode contracts
geometrically with rate `1-γ`, regardless of the switching schedule. If
`γ > 1`, the hypotheses force the weighted norm of every relevant mode to
vanish, so no separate upper bound on `γ` is needed. -/
theorem wnormSq_iterSwitch_le (π : S → ℝ) (Ps : ℕ → S → S → ℝ) (γ : ℝ)
  ...
  have hP1 : ∀ n x, ∑ y, Ps n x y = 1 := by
    intro m x
    obtain ⟨Q, _, hQ1, _, hPQ⟩ := hlazy m
    exact row_sum_of_lazy (Ps m) Q hQ1 hPQ x
  have hrev : ∀ n x y, π x * Ps n x y = π y * Ps n y x := by
    intro m x y
    obtain ⟨Q, _, _, hQrev, hPQ⟩ := hlazy m
    exact reversible_of_lazy π (Ps m) Q hQrev hPQ x y
  have hmeanZero : ∀ m : ℕ, ∑ x, π x * actSeq Ps m f x = 0 := by
    intro m
    induction m with
    | zero => simpa [actSeq] using hf0
    | succ m ih =>
        rw [actSeq]
        exact act_preserves_meanZero π (Ps m) (hP1 m) (hrev m)
          (actSeq Ps m f) ih
  have hnormNonneg : ∀ g : S → ℝ, 0 ≤ wnormSq π g := by
    intro g
    unfold wnormSq wip
    apply Finset.sum_nonneg
    intro x _
    rw [show π x * g x * g x = π x * (g x) ^ 2 by ring]
    exact mul_nonneg (hπ x) (sq_nonneg (g x))
  by_cases hγ1 : γ ≤ 1
  · have hnorm : ∀ m : ℕ,
      ...
      | succ m ih =>
          have hcontract :
              wnormSq π (act (Ps m) (actSeq Ps m f)) ≤
                (1 - γ) * wnormSq π (actSeq Ps m f) :=
            wnormSq_act_le_of_lazy_of_gap π (Ps m) γ hπ (hlazy m) (hgap m)
              (actSeq Ps m f) (hmeanZero m)
          have hnonneg1mγ : (0 : ℝ) ≤ 1 - γ := by linarith
          calc
            wnormSq π (actSeq Ps (m + 1) f) =
                wnormSq π (act (Ps m) (actSeq Ps m f)) := by rw [actSeq]
            _ ≤ (1 - γ) * wnormSq π (actSeq Ps m f) := hcontract
            _ ≤ (1 - γ) * ((1 - γ) ^ m * wnormSq π f) :=
                mul_le_mul_of_nonneg_left ih hnonneg1mγ
            _ = (1 - γ) ^ (m + 1) * wnormSq π f := by ring
    exact hnorm n
  · have hγgt : 1 < γ := lt_of_not_ge hγ1
    have hfirst := wnormSq_act_le_of_lazy_of_gap
      π (Ps 0) γ hπ (hlazy 0) (hgap 0) f hf0
    have hfNormZero : wnormSq π f = 0 := by
      nlinarith [hnormNonneg f, hnormNonneg (act (Ps 0) f)]
    have hzero : ∀ m : ℕ, wnormSq π (actSeq Ps m f) = 0 := by
      intro m
      induction m with
      | zero => simpa [actSeq] using hfNormZero
      | succ m ih =>
          have hcontract := wnormSq_act_le_of_lazy_of_gap
            π (Ps m) γ hπ (hlazy m) (hgap m)
              (actSeq Ps m f) (hmeanZero m)
          apply le_antisymm
          · simpa [actSeq, ih] using hcontract
          · exact hnormNonneg (actSeq Ps (m + 1) f)
    rw [hzero n, hfNormZero, mul_zero]

end Math.Probability.ReversibleDirichletContraction
