...
/-!
# Kakutani dichotomy for independent infinite product laws

For independent finite-alphabet steps, summable Hellinger defects give the
absolute-continuity direction of the Kakutani dichotomy.  The one-directional
theorem needs only support domination; applying it in both directions gives
mutual absolute continuity when the per-step supports agree.

The proof compares each infinite product with a finite-horizon hybrid by an
explicit density.  Cylinder approximation and a finite Hellinger
total-variation bound then control the infinite tail.  The mixture corollary
allows rates in `[0, 1)` and derives absolute continuity from square
summability and a uniform chi-squared bound.

The results concern independent products only.  No conclusion is asserted
for history-dependent kernels or controlled processes without independent
per-step laws.
-/


namespace Math.Probability.KakutaniProductDichotomy
...
/-- The Hellinger affinity of two mass functions on `I`:
`∑ i, √(f i * g i)`. -/
...
omit [MeasurableSpace I] [DiscreteMeasurableSpace I] in
/-- Under shared support and normalization of `f`, the affinity
...
omit [Fintype I] in
/-- **The finite-`N` product identity** behind `E_P[L N] = 1` and
...
/-- The horizon-`N` likelihood ratio, as an `ℝ≥0∞`-valued density:
...
`Measure.eq_infinitePi`; support domination (`hsupp`) is what cancels `ofReal (q)/ofReal (p) *
...
theorem productLaw_hybrid_eq_withDensity (p q : ℕ → I → ℝ)
  ...
  have hcancel : ∀ t x, ENNReal.ofReal (q t x) / ENNReal.ofReal (p t x) *
    ...
    exact le_of_eq (hsupp t x (le_antisymm h0 (hp0 t x)))
...
/-! ## The absolute-continuity direction of the Kakutani dichotomy -/

/-- **Kakutani dichotomy, product case, absolute-continuity direction.**
Independent finite-alphabet steps with dominated per-step support and summable
...
theorem product_mutuallyAC (p q : ℕ → I → ℝ)
  ...
  ⟨productLaw_absolutelyContinuous p q hp0 hq0 hp1 hq1
      (fun t i => (hsupp t i).mp) hρ,
    productLaw_absolutelyContinuous q p hq0 hp0 hq1 hp1
      (fun t i => (hsupp t i).mpr)
...
`q t i = (1 - δ t) * p t i + δ t * s t i` with mixing rates `δ t ∈ [0, 1)`, positive base
`p`, normalized direction `s`, uniform χ²-bound `C`, and square-summable rates, the baseline
and deviation product laws are mutually absolutely continuous.  Mixtures with `δ < 1` and
positive base automatically have identical one-step support. -/
theorem hdich_of_product (p s : ℕ → I → ℝ) (δ : ℕ → ℝ) (C : ℝ)
    (hp_pos : ∀ t i, 0 < p t i) (hp1 : ∀ t, ∑ i, p t i = 1)
    (hs0 : ∀ t i, 0 ≤ s t i) (hs1 : ∀ t, ∑ i, s t i = 1)
    (hδ : ∀ t, δ t ∈ Set.Ico (0 : ℝ) 1)
    (hC : ∀ t, ∑ i, (s t i - p t i) ^ 2 / p t i ≤ C)
    (hδ2 : Summable fun t => δ t ^ 2) :
    productLaw p ≪ productLaw (fun t i => (1 - δ t) * p t i + δ t * s t i) ∧
      productLaw (fun t i => (1 - δ t) * p t i + δ t * s t i) ≪ productLaw p := by
  ...
  have hρ : Summable fun t => 1 - affinity (p t) (q t) := by
    ...
    calc 1 - affinity (p t) (q t)
      ...
      _ = C / 2 * δ t ^ 2 := by ring
  have hsupp : ∀ t i, p t i = 0 ↔ q t i = 0 := by
    intro t i
    constructor
    · exact fun hzero => False.elim ((hp_pos t i).ne' hzero)
    · exact fun hzero => False.elim ((hq_pos t i).ne' hzero)
  have hmut := product_mutuallyAC p q (fun t i => (hp_pos t i).le)
    (fun t i => (hq_pos t i).le) hp1 hq1 hsupp hρ
  simpa only [hq] using And.intro hmut.2 hmut.1

end Math.Probability.KakutaniProductDichotomy
