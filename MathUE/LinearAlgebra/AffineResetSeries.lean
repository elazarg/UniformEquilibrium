...
Two consecutive reset phases are again one reset phase. The effective target
is a canonical convex combination of the two original targets, giving an
exact algebraic rule for eliminating an intermediate affine contraction.

Common pinned coordinates and common lower floors survive the contraction.
A coordinate pinned in only one phase need not survive; the formulas below
measure the resulting error exactly.
-/

noncomputable section

namespace Math.AffineResetSeries
...
/-- The canonical target weights sum to one whenever their denominator is
nonzero. -/
theorem resetSeriesWeights_sum_one
    {firstRatio secondRatio : ℝ}
...
theorem resetSeriesWeights_nonneg_sum_one
  ...
  · exact resetSeriesWeights_sum_one (ne_of_gt hden)
...
theorem resetSeriesEffectiveTarget_eq_of_common_pin
  ...
  have hsum := resetSeriesWeights_sum_one hden
  unfold resetSeriesEffectiveTarget
  rw [hfirst, hsecond]
  calc
...
theorem resetSeriesEffectiveTarget_eq_pin_iff_second_eq_pin
  ...
  have hweights := resetSeriesWeights_nonneg_sum_one
    hfirst1 hsecond0 hsecond1
  have hsecondWeight : 0 < resetSeriesSecondWeight firstRatio secondRatio :=
    div_pos (sub_pos.mpr hsecond1)
      (resetSeries_denominator_pos hfirst1 hsecond0 hsecond1)
  unfold resetSeriesEffectiveTarget
  rw [hfirst]
  constructor
  · intro heffective
...
theorem resetSeriesEffectiveTarget_sub_pin
  ...
  calc
    ...
    _ = resetSeriesSecondWeight firstRatio secondRatio *
        (secondTarget who - pinned) := by rw [hsum]; ring
...
theorem abs_resetSeriesEffectiveTarget_sub_pin
    ...
    |resetSeriesEffectiveTarget firstRatio secondRatio
      ...
      resetSeriesSecondWeight firstRatio secondRatio *
        |secondTarget who - pinned| := by
  rw [resetSeriesEffectiveTarget_sub_pin
    (ne_of_gt (resetSeries_denominator_pos hfirst1 hsecond0 hsecond1))
