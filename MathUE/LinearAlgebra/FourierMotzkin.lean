...
This module is adapted from EconCSLib's Fourier-Motzkin theorem-of-alternatives
formalization.
...
* `theorem_of_alternative`: finite theorem of alternatives for weak linear inequalities.
-/
...
omit [IsStrictOrderedRing 𝕜] in
/-- Inner sum at `Sum.inl k` collapses to the picked-out row:
`∑ i, L (inl k) i * g i = g k.val`. -/
