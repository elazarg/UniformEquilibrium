...
and nonnegative weighted total payoff exceeds weighted target by at most `slack`,
...
namespace Math.WeightedSaturationDefect
...
theorem coordinate_excess_le
    ...
    (weight_nonneg : ∀ j, j ≠ i → 0 ≤ weight j)
    (weight_i_pos : 0 < weight i)
  ...
  have weightedExcess :
    ...
    linarith
  apply (le_div_iff₀ weight_i_pos).2
...
theorem coordinate_mem_security_saturation_interval
  ...
  · have upperExcess := coordinate_excess_le weight target payoff i
      slack securityError weight_nonneg weight_i_pos
        (fun j _ => lower j) weightedCap
    linarith
...
theorem deviation_gain_le
  ...
  have deviatingExcess := coordinate_excess_le
    weight target deviating i slack securityError weight_nonneg weight_i_pos
      deviatingOthersFloor deviatingWeightedCap
  linarith

end Math.WeightedSaturationDefect
