...
collateral, default, or high-probability rules. There is no game content:
`d` and `Φ` are bare real sequences, not payoffs, strategies, or equilibrium
objects.
-/

noncomputable section

namespace Math.EscrowDrawdown
...
/-- Characterization: the drawdown is exactly the infimum of initial balances
keeping every prefix through `N` solvent. Feasibility at the empty prefix
already forces every such balance to be nonnegative. -/
theorem drawdown_eq_sInf (d : ℕ → ℝ) (N : ℕ) :
    drawdown d N = sInf {e : ℝ | ∀ n ≤ N, 0 ≤ e + prefixSum d n} := by
  apply le_antisymm
  · apply le_csInf
    · exact ⟨drawdown d N, fun n hn => add_prefixSum_nonneg d N hn⟩
    · intro e he
      exact drawdown_le_of_feasible d N e he
  · apply csInf_le
    · refine ⟨0, fun e he => ?_⟩
      simpa using he 0 (Nat.zero_le N)
    · exact fun n hn => add_prefixSum_nonneg d N hn

/-- If the first `n` increments of `d` telescope a potential `Φ`, their
prefix sum is the potential's net change from time `0`. -/
theorem prefixSum_eq_sub (d Φ : ℕ → ℝ) (n : ℕ)
    (hd : ∀ i < n, d i = Φ (i + 1) - Φ i) :
    prefixSum d n = Φ n - Φ 0 := by
  induction n with
  | zero => simp
  | succ n ih =>
      rw [prefixSum_succ,
        ih (fun i hi => hd i (hi.trans (Nat.lt_succ_self n))),
        hd n (Nat.lt_succ_self n)]
      ring
...
theorem drawdown_le_sub_inf' (d Φ : ℕ → ℝ) (N : ℕ)
    (hd : ∀ i < N, d i = Φ (i + 1) - Φ i) :
    drawdown d N ≤ Φ 0 - (Finset.range (N + 1)).inf' Finset.nonempty_range_add_one Φ := by
  apply Finset.sup'_le
  intro n hn
  have hn' : n ≤ N := Nat.lt_succ_iff.mp (Finset.mem_range.mp hn)
  rw [prefixSum_eq_sub d Φ n (fun i hi => hd i (hi.trans_le hn'))]
  have hinf : (Finset.range (N + 1)).inf' Finset.nonempty_range_add_one Φ ≤ Φ n :=
    Finset.inf'_le Φ hn
  linarith

/-- Potential-oscillation bound: if the increments before the horizon are
potential differences, the drawdown (hence the minimum escrow) is bounded by
the potential's oscillation over the horizon. -/
theorem drawdown_le_oscillation (d Φ : ℕ → ℝ) (N : ℕ)
    (hd : ∀ i < N, d i = Φ (i + 1) - Φ i) :
    drawdown d N ≤
...
potential has no slack: the worst prefix is the last one, and the drawdown equals the
...
theorem drawdown_eq_oscillation_of_antitone (d Φ : ℕ → ℝ) (N : ℕ)
    (hd : ∀ i < N, d i = Φ (i + 1) - Φ i) (hΦ : AntitoneOn Φ (Set.Iic N)) :
    drawdown d N =
      (Finset.range (N + 1)).sup' Finset.nonempty_range_add_one Φ -
        (Finset.range (N + 1)).inf' Finset.nonempty_range_add_one Φ := by
  ...
  · have hmem : N ∈ Finset.range (N + 1) := Finset.mem_range.mpr (Nat.lt_succ_self N)
    have hle : -(prefixSum d N) ≤ drawdown d N :=
      Finset.le_sup' (fun m => -(prefixSum d m)) hmem
    rw [prefixSum_eq_sub d Φ N hd] at hle
    linarith

end Math.EscrowDrawdown
