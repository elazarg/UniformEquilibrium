...
| Upward transport of quitting-game terminal exploitability | Unconditional quantitative transport theorem | [`HasTerminalExploitabilityGap.passivePlayerPadding_canonical`](UniformEquilibrium/Quitting/Terminal/PassivePlayerPaddingCanonical.lean) | Adding any nonempty finite passive player block preserves an all-behavior terminal exploitability gap with the exact factor penalty / (penalty + card J * canonicalWidth); the checked canonical corollary propagates nonexistence to the padded Sum player type. |
| Cumulative-charge payoff near-returns and infinite exact floor orbits | Conditional consumer and analytic dichotomy | [`uniformPayoff_or_summableChargeAllContinuePort`](UniformEquilibrium/Quitting/Bellman/Finite/PunishmentFloorInfiniteOrbitChargeDichotomy.lean) | A fixed lower bound on total exact-path absorption charge, together with arbitrarily close endpoint payoff vectors, yields a uniform-equilibrium payoff without a fixed charged edge. Every infinite exact punishment-floor orbit either closes through nonsummable cumulative charge or converges to a floor-safe all-Continue self-loop. A nonzero summable displacement retains one fixed signed terminal coalition with the sharp finite-label share; no paid-source restart is supplied. |
| Actual-profile paid cap ports and uniform-step obstruction | Conditional actual-source construction and exact boundary | [`HasTerminalExploitabilityGap.not_uniformPositivePaidCapDebtDropSelection`](UniformEquilibrium/Diagnostics/Quitting/StoppingLaw/Endpoint/ActualProfilePaidCapUniformStepObstruction.lean) | Under a positive terminal exploitability gap and positive global minimum debt, every literal behavioral profile supplies a full-gap paid cap port and its exhaustive, pairwise-disjoint exact trichotomy. Complete absorption and cap displacement satisfy D_* A <= D_source - D_* and D_* rho <= 2 R (D_source - D_*); every exact-minimum source is unconditionally a literal inert stall. At every positive triple of tolerances, one near-minimum literal profile makes every compatible full-gap paid-cap source/port simultaneously have small debt drop, absorption, and displacement. Hence no selector over all profiles can retain one fixed positive real debt drop. No minimum profile, well-founded or nonuniform regeneration, or inert-stall consumer is supplied. |
| Fin4 same-source paid/reset cap port | Unconditional actual-source composition theorem | [`QuittingTerminalExploitabilityWitness.nonempty_finFourSameSourcePaidResetCapPort`](UniformEquilibrium/Diagnostics/Quitting/StoppingLaw/Endpoint/FinFourSameSourcePaidResetCapPort.lean) | Every Fin 4 terminal exploitability witness and every prescribed reset owner with a disjoint two-player base supply one actual stationary profile and law which simultaneously carry the full-gap paid row, fixed-law reset target, and unchanged suffix of a cap-lifted summable port. The positive carrier minimum and separate reset-returned pair are retained. The cap lift bypasses that returned pair and supplies no prescribed-payoff path, restart, debt descent, or uniform payoff. |
| Fin4 singleton-base same-law reset and double paid cap port | Unconditional actual-source reduction theorem | [`FinFourQuantitativeFullSupportHardResidual.nonempty_paidCapDoublePort`](UniformEquilibrium/Diagnostics/Quitting/Collision/SingletonPacket/SingletonBaseResetRepairPaidCapDoublePort.lean) | Every bounded Fin 4 quantitative hard residual and prescribed singleton owner produce one actual singleton-base stationary source with three solved free coordinates, a unique full-gap paid debtor, a quantitative strict-superset atom, and a same-law fixed reset dispatch. Its literal owner repair carries a second paid row at a distinct observer. Cap-lifting both profiles from the same positive minimum leaves quantitative debt descent on one profile or literal inert stalls on both; no descent regeneration or identification of the two inert laws is supplied. |
...
| Componentwise weighted-potential affine and quadratic quitting tables | Unconditional family theorem | [`quittingGame_exists_uniformPayoff_of_componentwiseWeightedPotential`](UniformEquilibrium/Quitting/Stationary/ComponentwiseWeightedPotential.lean) | Affine own-membership gains that are positively symmetrizable inside every SCC of their directed influence graph produce a literal sure-exit coalition, a pure stationary exact terminal Nash profile against every unilateral behavioral deviation, and a uniform-equilibrium payoff. Cross-component influences and passive reward coordinates are unrestricted. The quadratic adapter includes equal active pair coefficients and sign-frustrated reciprocal negative cycles. This sufficient solved chamber neither characterizes all quadratic tables nor supplies its hypothesis for an arbitrary hard residual. |
...
| AGKRS divergent exceptional-owner source closure | Unconditional source consumer | [`QuittingUniqueExceptionalOwnerSource.instantPunishment_or_wellSupported`](UniformEquilibrium/Quitting/Classification/Existence/DivergentExceptionalOwnerS3Dispatch.lean) | Every actual unique-exceptional-owner source with divergent selected horizons yields literal S.2 or well-supported S.3; source Nash retains arbitrary unilateral behavioral deviations, while the explicit negative-owner solo sequence is not claimed terminal Nash. The residual adapter removes the entire divergent negative exceptional-owner class. The sharper Literature capstone is recorded separately; unconditional theorem3_4 remains open. |
| Positive-debt attained terminal-semantic image is not closed | Unconditional topological obstruction | [`attainable_inter_debt_ge_one_not_closed`](UniformEquilibrium/Diagnostics/Quitting/PositiveDebtTerminalSemanticNonattainment.lean) | A rational two-player table has actual semantic pairs of debt 1 + 1/(n+1) converging to an unattained carrier point of debt 1; the full clock-payoff face is the open interval of positive atom caps, and every one-player attained semantic image is closed. The example quantifies over arbitrary behavioral profiles, but its global minimum debt is 0. It rules out attainment from pointwise positive debt alone and does not address the globally minimal D = D_* > 0 fiber or advance the quitting conjecture. |
| Uniform payoffs as diagonal terminal-semantic carrier points | Unconditional semantic equivalence | [`isUniformEquilibriumPayoff_iff_diagonal_mem_terminalSemanticCarrier`](UniformEquilibrium/Quitting/Classification/Existence/UniformPayoffTerminalSemanticCarrier.lean) | For every inhabited finite player type and fixed payoff target, the target is a uniform-equilibrium payoff exactly when its diagonal pair belongs to the closure of executable terminal payoff/unrestricted-best-response pairs. Existentially, target-free approximate-equilibrium existence is exactly diagonal-carrier nonemptiness; the normalized arbitrary-Never AGKRS premise has the same characterization. Carrier membership need not be attained by one profile. This is a semantic equivalence, not a stationary, instant-punishment, or sequentially-perfect classification, and it reformulates rather than proves the unconditional quitting conjecture. |
| AGKRS prioritized attachment reduction and exact residual normal form | Unconditional source reduction | [`sourcePhantom_or_positiveSingletonDefect`](UniformEquilibrium/Quitting/Classification/Existence/PrioritizedAttachmentSingletonDefect.lean) | At every positive prioritized scale, the positive-absorption attachment arm is consumed: a finite sure-exit row contradicts the instant-punishment priority after a checked punishment-floor lift, while an infinite reached-row spine contracts through the support--Bellman boundary. The exact surviving normal form is the source-faithful all-Continue positive-rho residual or the positive-singleton defect, on the same table and scale with all four priority negations retained. Neither survivor has an S.1/S.2/S.3 consumer; this does not close AGKRS Theorem 3.4 or advance the unconditional finite-quitting conjecture. |
| AGKRS positive-joint endpoint, diagonal carrier, and ballistic reductions | Unconditional source reductions and payoff identification; conditional Literature capstone | [`wellSupported_or_endpointBallisticBoundary`](UniformEquilibrium/Quitting/Classification/Existence/PositiveJointSummablePortBallisticDispatch.lean) | Every reached positive-joint punishment endpoint is diagonal in the literal terminal-semantic carrier, so its prescribed coordinate is a specific uniform-equilibrium payoff. Every pair on its canonical exact semantic-prefix orbit is likewise diagonal, and every supplied summable-port limit is a diagonal carrier point and specific uniform-equilibrium payoff target. This does not place either target in S.1, S.2, or S.3 and gives no new unconditional quitting-conjecture advance. Nonsummable absorption yields well-supported S.3; a summable port otherwise yields S.1 or the common all-Continue phantom/positive-singleton defect. The provenance-retaining ballistic dispatch closes positive charge plus endpoint return through S.3; under S.3 failure it leaves zero-charge rigidity or a displaced signed port. Neither survivor has a classification consumer. The Literature capstone and unconditional theorem3_4 remain open. |
| Simon normalized-motion stationary-prefix producer | Unconditional alternative theorem | [`instant_or_stationarilyGenerated_or_normalizedMotionLowerBound`](UniformEquilibrium/Quitting/Classification/SimonFiniteOrbit/NormalizedMotionStationaryPrefixProducer.lean) | For every nonempty finite quitting game: instant punishment, stationarily generated approximate equilibria, or one fixed positive normalized-motion lower bound on every rational support-local row. Arbitrarily small actual positive-absorption normalized-motion rows produce the stationarily generated branch against arbitrary behavioral deviations. The lower-bound arm asserts no row feasibility, and the theorem does not prove the positive-solo clause, a common compact-carrier scale, Simon Lemma 2, or a finite orbit. |
