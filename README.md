...
| Literal paid-row exact-port alternative | Conditional actual-source alternative | [`QuittingPaidRowFloorSafeSource.exists_markedExactOrbit_alternative_of_witness`](UniformEquilibrium/Diagnostics/Quitting/StoppingLaw/Endpoint/PaidRowExactPortAlternative.lean) | An attained paid first-disagreement profile whose prescribed payoff explicitly dominates every behavioral punishment value generates one literal root-prefixed exact floor orbit retaining that profile as a suffix. Nonsummable charge closes through cumulative payoff recurrence. Summable charge converges in full terminal semantics to an all-Continue fixed port; under a terminal exploitability witness the unchanged paid suffix has positive limiting reach. The paid source's floor inequality and a restart from the summable port remain unproved. |
