...
| Actual-profile paid cap ports and minimum-fiber contraction | Conditional actual-source construction and exact boundary | [`HasTerminalExploitabilityGap.nonempty_actualProfilePaidCapMinimumApproximation`](UniformEquilibrium/Diagnostics/Quitting/StoppingLaw/Endpoint/ActualProfilePaidCapMinimumApproximation.lean) | Under a positive terminal exploitability gap and positive global minimum debt, every literal behavioral profile supplies a full-gap paid cap port and its exhaustive, pairwise-disjoint exact trichotomy. Complete absorption and cap displacement satisfy D_* A <= D_source - D_* and D_* rho <= 2 R (D_source - D_*); every exact-minimum source is unconditionally a literal inert stall. Carrier density gives actual full-gap ports with source debt tending to D_* and both A and rho tending to zero. No minimum profile, regenerated descent source, well-founded rank, or inert-stall consumer is supplied. |
...
| AGKRS positive-joint endpoint, phantom, and ballistic reductions | Unconditional source reductions and conditional Literature capstone | [`wellSupported_or_endpointBallisticBoundary`](UniformEquilibrium/Quitting/Classification/Existence/PositiveJointSummablePortBallisticDispatch.lean) | Every reached positive-joint punishment endpoint has a canonical exact semantic-prefix orbit. Nonsummable absorption yields well-supported S.3; a summable port yields S.1 or the common nonzero all-Continue phantom, equivalently the existing positive-singleton defect at each tolerance. The provenance-retaining ballistic dispatch additionally closes every positive-charge port returning to its endpoint through literal S.3; under S.3 failure it leaves exactly a zero-charge constant all-Continue orbit or a quantitatively displaced port with one fixed signed terminal coalition and the canonical reward-bound mass scale. Neither surviving arm is consumed, and real displacement is not a well-founded rank. Forgetting to the corrected pointwise residual discards endpoint, no-sure-exit, and orbit provenance and returns a raw residual, not a prioritized one. The Literature capstone still leaves the prioritized and summable-port consumers; unconditional theorem3_4 remains open. |
