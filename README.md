...
| Actual-source paid cap-lifted summable port | Conditional actual-source theorem | [`nonempty_capLiftedSummablePort_of_tangentFamily`](UniformEquilibrium/Diagnostics/Quitting/StoppingLaw/Endpoint/PaidCapLiftedSummablePort.lean) | At positive global minimum terminal-semantic debt, every attained positive paid first-disagreement source generates exact cap-Nash prefixes without a prescribed-payoff floor hypothesis. Their absorption is summable with an exact debt budget and a uniform debt-ratio lower bound on reaching the unchanged paid suffix. Every finite prefix carries the correspondingly shifted paid row with uniform positive gain, while both cap and actual terminal semantics converge to all-Continue ports. No restart, cumulative-charge near-return, or uniform-payoff consumer is supplied. |
