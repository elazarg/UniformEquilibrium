...
| Four-player quitting counterexample residual | Unconditional alternative theorem | [`uniformPayoff_or_quantitative_fullSupport_fullNormalCore_of_finFour`](UniformEquilibrium/Diagnostics/Quitting/Collision/SingletonPacket/NormalTerminalGapConstrainedStationary.lean) | Every Fin 4 reward table has a uniform-equilibrium payoff or, on the same table, a terminal exploitability witness, full recursive normal core, all-player punishment normality, and a normalized singleton packet with full support and an explicit positive coordinate floor. The residual is not solved. |
