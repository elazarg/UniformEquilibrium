...
| AGKRS positive-joint endpoint and phantom reduction | Unconditional source reductions and conditional Literature capstone | [`wellSupported_or_stationary_or_allContinuePhantom`](UniformEquilibrium/Quitting/Classification/Existence/PositiveJointSummablePortPhantomReduction.lean) | Every reached positive-joint punishment endpoint has a canonical exact semantic-prefix orbit. Nonsummable absorption yields well-supported S.3; a summable port yields S.1 or the common nonzero all-Continue phantom, equivalently the existing positive-singleton defect at each tolerance. Forgetting to the corrected pointwise residual discards endpoint, no-sure-exit, and orbit provenance and returns a raw residual, not a prioritized one, so no branch consumer or closure follows. The generic diagonal-carrier theorem gives a uniform payoff if (v,v) belongs to the terminal-semantic carrier, but such membership is not proved for the port limit. The Literature capstone still leaves the prioritized and summable-port consumers; unconditional theorem3_4 remains open. |
