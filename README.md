...
| Prescribed-owner reset and stationary handoff alignment | Conditional structural theorem | [`QuittingTerminalExploitabilityWitness.exists_finFour_prescribedOwner_resetDispatch`](UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticFinFourPrescribedOwnerResetAlignment.lean) | At a global minimum terminal-semantic carrier pair on Fin 4, any chosen positive debtor can be the owner of an actual complementary-pair stationary target with zero owner debt, unit opponent incidence, and the checked fixed-law reset dispatch. Independently, any preselected label can own an actual singleton-base stationary paid handoff with a uniform positive owner-floor gap. The common label does not identify the two profiles, terminal laws, returned observer, payoff vectors, or chronology; the all-Continue reset wall and paid-return connector remain open. |
...
| Fin4 charged blocker gate and exact repayment | Conditional quantitative obstruction theorem | [`pairPremium_or_every_exactRepayment`](UniformEquilibrium/Diagnostics/Quitting/Collision/Toggles/ChargedSoloBlockerGateRepayment.lean) | Every fixed-charge exact carrier-root family either has positive-liminf semantic debt descent or converges along a strict subsequence to one mixed solo debtor and a punishment-normal outsider attaining the maximal joining gap. The actual gate row extends to exact punishment-floor orbits on which a fixed pair premium or fixed repayment in the blocker coordinate is unavoidable. The synthetic blocker tail is not asserted to be a carrier point, no collision-mass producer follows from the premium, and the other payoff coordinates need not return. |
| Fin4 solo-wall debt descent and finite-spine exclusion | Conditional structural reduction | [`not_exists_outwardUniformSoloCarrierChain_of_normal`](UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticFinFourSoloWallDispatch.lean) | From a unique-debtor exact solo gate, a uniform opponent-absorption floor gives strict carrier-debt descent; otherwise finite solo prefixes reach a first outsider wall. Arbitrarily long finite uniform-solo windows compactify to an infinite spine, and punishment normality excludes the corresponding infinite outward chain. A pair premium at the wall yields the checked singleton-base handoff or an outsider pair-join. The pair-base stationary two-debtor source promised from that join is not proved, so the full packet remains in revisit and no payoff return follows. |
