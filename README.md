...
| Six-player one-pair mass ledger and direct target lock | Unconditional quantitative theorem | [`integerReward_exploitability_ge`](UniformEquilibrium/Quitting/Paths/SixPlayerOnePairMassTargetLock.lean) | A complete Fin 6 integer table forces terminal exploitability at least 31(1-a)/66 and hence the exact first-pair and leftover bounds; bounded outsider-coordinate completions retain robust estimates. Every such direct cross-penalty completion also has a pure exact target equilibrium and uniform payoff, so it cannot force the second pair. The checked second-pair upper bound still assumes a supplied clock inequality; no arbitrary-profile clock adapter is proved here. |
