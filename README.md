...
| Pair-base paid/reset payoff alignment | Conditional structural theorem | [`QuittingTerminalExploitabilityWitness.exists_finFour_pairBasePaidResetDispatch_payoffAligned`](UniformEquilibrium/Diagnostics/Quitting/Collision/SingletonPacket/PairBasePaidResetPayoffAlignment.lean) | On Fin 4, any prescribed owner and disjoint two-player sure-Quit base yield one actual stationary target and law with zero owner debt, unit incidence from a base opponent, and a terminal-gap paid row whose debtor lies in that base. From a positive global-minimum source the target enters the fixed-law reset dispatch, whose returned pair has exactly the target profile's prescribed payoff because both retain the same complete terminal law. The paid debtor is not the reset owner, and cap-Nash at the returned envelope does not supply an endpoint-Nash punishment-floor edge or return path. A checked closure-seam interface isolates those remaining data and would imply a uniform payoff. |
