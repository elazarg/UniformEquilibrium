...
| Paid cap-port exact trichotomy | Conditional exact trichotomy with a branchwise consumer | [`exactTrichotomy`](UniformEquilibrium/Diagnostics/Quitting/StoppingLaw/Endpoint/PaidCapPortExactTrichotomy.lean) | Every supplied actual paid cap-lifted summable port lies in exactly one of three pairwise-disjoint branches. Positive total absorption with zero cap displacement produces cumulative admissible payoff near-returns and a uniform-equilibrium payoff. Positive displacement gives the exact D_* rho / (2M) semantic-debt drop. Zero absorption makes every literal cap root all Continue and preserves the whole semantic pair and full paid row without loss. The descent has no regenerated source or well-founded consumer, and the inert branch remains open. |
