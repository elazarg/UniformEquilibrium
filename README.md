...
| Literal strict finite odd blocker cores with arbitrary outside-player payoffs | Conditional family theorem | [`isUniformEquilibriumPayoff_of_literalStrictFiniteOddBlockerCore`](UniformEquilibrium/Quitting/Classification/Existence/FiniteOddBlockerCoreRowAdapter.lean) | For every embedded odd cyclic core of any finite size at least three, passive core-continuation rows and uniform strict blocker switches yield an exact stationary all-behavior uniform-equilibrium payoff. Arbitrarily many outside players and every outside-player reward coordinate remain unrestricted. |
