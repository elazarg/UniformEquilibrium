...
theorem lemma2_1_part2 (G : QuittingGame)
  ...
  have hgenerated2007 :
      ¬Literature.Simon2007.HasStationarilyGeneratedApproximateEquilibria G :=
    fun h => hgenerated h
  have hinstant2007 : ¬Literature.Simon2007.HasInstantApproximateEquilibria G :=
    fun h => hinstant ((instantApproximateEquilibria_iff_simon2007 G).mpr h)
  obtain ⟨rate, hrate, hrateOne, hbound⟩ :=
    exists_correctedUniformMotionAt_of_not_branches G hgenerated2007 hinstant2007
  refine ⟨rate, hrate, hrateOne.le, ?_⟩
  intro tail row hnear hrational hrow
  have hnear2007 : Literature.Simon2007.NearFeasible G 1 tail := by
    obtain ⟨feasible, hfeasible, hdistance⟩ := hnear
    exact ⟨feasible, hfeasible, (norm_le_euclideanNorm (tail - feasible)).trans hdistance⟩
  obtain ⟨hmotion, hsurvival⟩ := hbound tail hnear2007 hrational row hrow
  exact ⟨hmotion.trans (norm_le_euclideanNorm _), hsurvival⟩
...
theorem lemma2_1_part2_compact (G : QuittingGame)
  ...
  have hinstant2007 : ¬Literature.Simon2007.HasInstantApproximateEquilibria G :=
    fun h => hinstant ((instantApproximateEquilibria_iff_simon2007 G).mpr h)
  obtain ⟨rate, hrate, hrateOne, hbound⟩ :=
    exists_compactMotionParameter_of_not_branches G hgenerated2007 hinstant2007 K hK
  refine ⟨rate, hrate, hrateOne.le, ?_⟩
  intro tail htail row hrational hrow
  obtain ⟨hmotion, hsurvival⟩ := hbound tail htail row hrational hrow
  exact ⟨hmotion.trans (norm_le_euclideanNorm _), hsurvival⟩
