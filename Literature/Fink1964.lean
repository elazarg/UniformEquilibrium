...
def maxDiscountNNReal (P : Game ι) [Fintype ι] [Nonempty ι] : ℝ≥0 :=
  ⟨P.maxDiscount, by
    obtain ⟨i⟩ := ‹Nonempty ι›
    exact (P.discount_nonneg i).trans
...
theorem property_b
  ...
  apply mul_le_mul_of_nonneg_left _ (P.discount_nonneg who)
  rw [← expect_sub]
...
theorem lemma_5
  ...
  have hβ : Tendsto (fun n => P.beta (xn n)) atTop (𝓝 (P.beta x)) :=
    (P.continuous_beta.tendsto x).comp hx
  have htriple : Tendsto
      (fun n => (xn n, yn n, P.beta (xn n))) atTop
      (𝓝 (x, y, P.beta x)) :=
    hx.prodMk_nhds (hy.prodMk_nhds hβ)
  have hf :=
    (P.property_a_continuous.tendsto (x, y, P.beta x)).comp htriple
  have hf' : Tendsto (fun n => P.beta (xn n)) atTop
      (𝓝 (P.f x y (P.beta x))) :=
    hf.congr fun n => hphi n
  exact tendsto_nhds_unique hf' hβ
