...
private theorem expectedStagePayoff_constantTrueFrom_ge_one
  ...
  intro history
  unfold StochasticGame.stageEUAt StochasticGame.stageActionDist
  change 1 ≤ prisonersDilemma.kernel.mixedExtension.eu
    (fun player => Function.update profile who
      (constantActionFrom prisonersDilemma profile who start true)
        player time history) who
  cases who
  · rw [prisonersDilemma_mixedEU_false]
    have hp : ((Function.update profile false
        (constantActionFrom prisonersDilemma profile false start true))
    ...
    have hq := ENNReal.toReal_mono ENNReal.one_ne_top
      (PMF.coe_le_one
        ((Function.update profile false
          (constantActionFrom prisonersDilemma profile false start true))
          true time history) true)
    rw [hp]
    norm_num at hq ⊢
    linarith
  · rw [prisonersDilemma_mixedEU_true]
    have hq : ((Function.update profile true
        (constantActionFrom prisonersDilemma profile true start true))
...
private theorem prisonersDilemma_one_third_mass_le_futureMass
  ...
  have hrowSummable : Summable rowTerm := by
    ...
    dsimp only [rowTerm]
    rw [abs_sub_comm]
    exact (abs_sub_le _ _).trans (add_le_add
      (prisonersDilemma.repeatedGame.abs_expectedStagePayoff_le
        (fun state action => hbound (state, action, false)) profile
        prisonersDilemma.repeatedInitial (start + (offset + 1))) (by norm_num))
  ...
  rw [← hsum]
  change _ ≤ (∑' offset, rowTerm offset) + ∑' offset, columnTerm offset
...
private theorem discountedPayoff_one_eq_mixedPayoff_initial
  ...
  rw [show (∑' time : ℕ, 0 ^ time *
      G.repeatedGame.expectedStagePayoff profile PUnit.unit time who) =
      G.repeatedGame.expectedStagePayoff profile PUnit.unit 0 who by
    rw [tsum_eq_single 0]
    · simp
    · intro time htime
      have hpositive : 0 < time := Nat.pos_of_ne_zero htime
      simp [Nat.ne_of_gt hpositive]]
  exact expectedStagePayoff_zero_eq_mixedPayoff_initial G profile who
