...
/-! The omitted nonstationarity assertions are proved by the production
boundary-table analysis. -/

/-- For all sufficiently small positive errors, no terminal approximate
equilibrium can remain uniformly close to all-Continue at every stage. -/
  ...
  ∃ ε₀ : ℝ, 0 < ε₀ ∧ ∀ ε, 0 < ε → ε < ε₀ →
    ¬ ∃ roots : RootSequence (ι := Fin 4),
      ...
      epsilonEquilibrium SolanVieilleBoundary.boundaryReward ε
        (profile SolanVieilleBoundary.boundaryReward roots) := by
  simpa [epsilonEquilibrium, profile,
    SolanVieilleBoundary.IsBoundaryTerminalApproxNash,
    SolanVieilleBoundary.boundaryRootSequenceProfile] using
      SolanVieilleBoundary.no_nearAllContinue_terminalApproximateEquilibrium

/-- For all sufficiently small positive errors, the boundary table has no
stationary terminal approximate equilibrium. -/
theorem figure2_no_stationary_epsilon :
  ∃ ε₀ : ℝ, 0 < ε₀ ∧ ∀ ε, 0 < ε → ε < ε₀ →
    ¬ ∃ root : Fin 4 → PMF Bool,
      epsilonEquilibrium SolanVieilleBoundary.boundaryReward ε
        (stationaryProfile SolanVieilleBoundary.boundaryReward root) := by
  simpa [epsilonEquilibrium, stationaryProfile,
    SolanVieilleBoundary.IsBoundaryTerminalApproxNash,
    SolanVieilleBoundary.boundaryStationaryProfile] using
