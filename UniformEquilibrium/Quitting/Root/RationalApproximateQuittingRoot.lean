...
theorem exists_rational_quittingRootTotalNashDefect_lt
  ...
  choose raw hrawMono hrawBelow hrawTendsto using fun player =>
    Real.exists_seq_rat_strictMono_tendsto ((exactSimplex player).weights true)
  ...
  have hprobabilityTendsto : ∀ player,
      Tendsto (fun time => ((probability player time : ℚ) : ℝ)) atTop
        (nhds ((exactSimplex player).weights true)) := by
    intro player
    let clamp : ℝ → ℝ := fun value => max 0 (min 1 value)
    have hclampContinuous : Continuous clamp :=
      continuous_const.max (continuous_const.min continuous_id)
    have hlimitZero : 0 ≤ (exactSimplex player).weights true :=
      (exactSimplex player).weights_nonneg true
    have hlimitOne : (exactSimplex player).weights true ≤ 1 :=
      (exactSimplex player).weights_apply_le_one true
    have hclampLimit : clamp ((exactSimplex player).weights true) =
        (exactSimplex player).weights true := by
      dsimp only [clamp]
    ...
    change Tendsto (fun time => clamp ((raw player time : ℚ) : ℝ)) atTop
      (nhds ((exactSimplex player).weights true)) at hclamped
  ...
  let approximate : ℕ → QuittingRootSimplex ι := fun time player =>
    ⟨Finsupp.equivFunOnFinite.symm fun action =>
        if action then (probability player time : ℝ)
        else 1 - (probability player time : ℝ),
      (fun action => by
        cases action with
        | false =>
            change 0 ≤ 1 - (probability player time : ℝ)
            have hcast : (probability player time : ℝ) ≤ 1 := by
              exact_mod_cast hprobabilityOne player time
            linarith
        | true =>
            change 0 ≤ (probability player time : ℝ)
            exact_mod_cast hprobabilityZero player time),
      (by
        rw [Finsupp.equivFunOnFinite_symm_sum, Fintype.sum_bool]
        simp only [Bool.false_eq_true, ↓reduceIte]
        ring)⟩
  have approximate_weights (time : ℕ) (player : ι) (action : Bool) :
      (approximate time player).weights action =
        if action then (probability player time : ℝ)
        else 1 - (probability player time : ℝ) := by
    dsimp only [approximate]
    exact Finsupp.equivFunOnFinite_symm_apply_apply _ _
  have happroximate : Tendsto approximate atTop (nhds exactSimplex) := by
    ...
    cases action with
    | false =>
        change Tendsto (fun time => (approximate time player).weights false) atTop
          (nhds ((exactSimplex player).weights false))
        have hsum := (exactSimplex player).total_of_fintype
        rw [Fintype.sum_bool] at hsum
        have hfalse : (exactSimplex player).weights false =
            1 - (exactSimplex player).weights true := by
          apply eq_sub_iff_add_eq.mpr
          simpa only [add_comm] using hsum
        rw [show (fun time => (approximate time player).weights false) =
            (fun time => 1 - (probability player time : ℝ)) by
          funext time
          rw [approximate_weights]
          rfl]
        rw [hfalse]
        exact tendsto_const_nhds.sub (hprobabilityTendsto player)
    | true =>
        change Tendsto (fun time => (approximate time player).weights true) atTop
          (nhds ((exactSimplex player).weights true))
        rw [show (fun time => (approximate time player).weights true) =
            (fun time => (probability player time : ℝ)) by
          funext time
          rw [approximate_weights]
          rfl]
  ...
  exact approximate_weights time player true
