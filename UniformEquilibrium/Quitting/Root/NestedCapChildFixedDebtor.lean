...
/-- Summable positive-survival source roots choose one positive floor before
the child depth is requested.  At each requested depth, one outsider and one
cap response are then selected once and that label stays a debtor at every
later actual child.  The companion theorem above supplies its literal copied
response and exact gain identity. -/
theorem HasTerminalExploitabilityGap.exists_positiveFloor_fixedDebtor_on_capChildren
    (reward : {S : Finset ι // S.Nonempty} → Payoff ι)
    ...
    (hbase : quittingTerminalPayoff reward
        (quittingPureTimeCapChild reward (profiles 0) owner 0) owner =
      quittingContinuationBestResponseValue reward (profiles 0) owner) :
    ∃ prefixFloor, 0 < prefixFloor ∧ 0 < prefixFloor * gap ∧
      ∀ start, ∃ (who : ι) (choice : Option ℕ),
        who ≠ owner ∧
          ...
          gap ≤ quittingTerminalDeviationDebt reward
            (quittingPureTimeCapChild reward (profiles start) owner start) who ∧
          ∀ fuel, prefixFloor * gap ≤
            quittingTerminalDeviationDebt reward
              (quittingPureTimeCapChild reward (profiles (start + fuel)) owner
                (start + fuel)) who := by
  obtain ⟨prefixFloor, hfloorPositive, hfloor⟩ :=
    exists_pos_le_jointSurvivalPrefix_of_summable_marginalHazard
      roots hsummable hpositive
  refine ⟨prefixFloor, hfloorPositive, mul_pos hfloorPositive hgap, fun start => ?_⟩
  obtain ⟨who, choice, hwho, hchoice, hcap, hdebt, hfuture⟩ :=
    hexploit.exists_fixedOutsiderResponse_for_all_capChildren
      reward hgap profiles roots owner hnested hexact hpositive hbase hfloor start
  exact ⟨who, choice, hwho, hchoice, hcap, hdebt,
    fun fuel => (hfuture fuel).2⟩
