...
* Positive margin sum with a negative margin calls on a *firing step* — a step
  ...
  reduce to nonpositivity of the join margins along the same progression.
...
distance and its complement — hence two independent cap pairs.  Both pockets
of the collider completion are settled outright in
...
* `exists_uniformEquilibriumPayoff_of_isFiringStep` — the firing-step branch
* `isQuittingSureExitSet_singleton_of_isCirculantPairTable` — the solo exit
  branch
...
* `exists_uniformEquilibriumPayoff_of_unique_nonneg` — three negative margins
  are resolved with no further hypothesis
