...
other players, `QuittingDeletedPlayer owner`.  This module supplies the
naturality which is absent from the equivalence-only player-reindexing API of
`UniformEquilibrium/Quitting/Classification/PlayerReindex.lean`: a behavioral
profile of the reduced game lifts to the original game by making `owner`
...
Under `QuittingOwnerJoinAntitone` the deleted owner also has exactly zero
