...
/-!
# Averaged security-restart deadline comparison

The actual security gain is already averaged over fresh private restart
randomness. The raw N/F/J rows compare it with the unchanged advancing gain
on every deterministic source tuple and outsider deadline.
-/
...
theorem deadlineSecurity_allEvaluatedGains_zero_of_first_lt
    ...
    cappedClockActualEvaluatedOutsideGain reward evaluation times
      ...
      ∀ i, cappedClockActualEvaluatedChildGain reward evaluation times
          (some deadline) i = 0 ∧
        deadlineSecurityActualEvaluatedChildGain reward evaluation times
  ...
  | coe firstTime =>
      ...
      · intro i
        ...
        · exact deadlineSecurityActualEvaluatedChildGain_of_first_lt
            reward evaluation times deadline i hafter

/-- An outsider Never clock changes no experiment. -/
theorem deadlineSecurity_allEvaluatedGains_zero_of_deadline_none
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
    (evaluation : WithTop ℕ → ℝ)
    (times : ι → Option ℕ) :
  ...
  · intro i
    simp [cappedClockActualEvaluatedChildGain, hcap,
      deadlineSecurityActualEvaluatedChildGain_none]
...
theorem deadlineSecurity_neverRow_pointwise
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
  ...
  have hwithdraw (i : ι) :
      deadlineSecurityActualEvaluatedChildGain reward evaluation times
        (some deadline) i = 0 := by
    apply deadlineSecurityActualEvaluatedChildGain_of_ne
    intro hclock
...
theorem deadlineSecurity_futureRow_pointwise
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
    (certificate : DeadlineSecurityRewardCertificate reward)
    (evaluation : WithTop ℕ → ℝ)
    (evaluation_nonneg : ∀ clock, 0 ≤ evaluation clock)
...
theorem deadlineSecurity_joinRow_pointwise
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
    (certificate : DeadlineSecurityRewardCertificate reward)
    (evaluation : WithTop ℕ → ℝ)
    (evaluation_nonneg : ∀ clock, 0 ≤ evaluation clock)
  ...
  calc
          ...
          (certificate.advanceWeight i *
              (reward ⟨cappedClockChildCoalition (insert i A),
                ...
                reward ⟨cappedClockChildCoalition A,
                  cappedClockChildCoalition_nonempty hA⟩ (some i)) +
            certificate.withdrawalWeight i *
              deadlineSecurityGainFloor reward i A hA) := hrow
    _ ≤ ∑ i, (certificate.advanceWeight i *
      ...
      intro i _
      have hfloor := deadlineSecurityActualEvaluatedChildGain_tie_floor
            ...
            (certificate.advanceWeight i *
                (reward ⟨cappedClockChildCoalition (insert i A),
                  ...
                  reward ⟨cappedClockChildCoalition A,
                    cappedClockChildCoalition_nonempty hA⟩ (some i)) +
              certificate.withdrawalWeight i *
                deadlineSecurityGainFloor reward i A hA) =
          certificate.advanceWeight i *
              cappedClockActualEvaluatedChildGain reward evaluation times
                (some deadline) i +
            certificate.withdrawalWeight i *
              (evaluation deadline * deadlineSecurityGainFloor reward i A hA) := by
          rw [hcapEq]
          ring
...
theorem deadlineSecurityActualEvaluatedOutsideGain_le_weighted_childGains
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
    (certificate : DeadlineSecurityRewardCertificate reward)
    (evaluation : WithTop ℕ → ℝ)
    (evaluation_nonneg : ∀ clock, 0 ≤ evaluation clock)
  ...
  cases deadline with
  | none =>
      have h := deadlineSecurity_allEvaluatedGains_zero_of_deadline_none
  ...
  | some time =>
      ...
      · have h := deadlineSecurity_allEvaluatedGains_zero_of_first_lt
      ...
      · by_cases htie : first = (time : WithTop ℕ)
        · exact deadlineSecurity_joinRow_pointwise reward certificate evaluation
        ...
        · have hbefore : (time : WithTop ℕ) < first :=
          ...
          | top =>
              exact deadlineSecurity_neverRow_pointwise reward certificate
                evaluation evaluation_nonneg times time (by simpa [first] using hfirst)
          | coe firstTime =>
              exact deadlineSecurity_futureRow_pointwise reward certificate
