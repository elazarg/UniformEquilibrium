...
/-!
# Bounded expectation of the deadline-withdrawal comparison

The deterministic D-N/F/J inequality is integrated over the independent
source tuple and outsider deadline. The private mixed response is identified
with a legal independent-product marginal before any behavioral cap is used.
-/
...
theorem expect_deadlineWithdrawal_pointwise_le
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
        ...
        calc
          ...
          _ ≤ certificate.advanceWeight i * gainBound +
            ...
            · exact mul_le_mul_of_nonneg_left
                (abs_deadlineEvaluatedGain_le reward evaluation
                  evaluation_nonneg evaluation_antitone
                  (withdrawnChildParentClocks sample.1 sample.2 i)
                  (quietParentClocks sample.1) (some i))
  ...
  intro sample
  exact deadlineWithdrawalActualEvaluatedOutsideGain_le_weighted_childGains
