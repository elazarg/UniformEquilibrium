...
/-!
# The private deadline mixture as a parent stopping-law product

The operation coin sees the selected child's original clock and an
independently sampled outsider deadline, but no opponent clock. Its resulting
parent tuple therefore has the actual legal one-child product stopping law.
-/
...
theorem deadlineMixedPrivateReplacement_parentMarginal
  ...
  calc
    _ = ((pmfPi childLaws).bind (fun times =>
        ...
        simpa only [PMF.map_comp, Function.comp_apply] using congrArg
          ...
          hmaps
    _ = _ := deadlineMixedPrivateReplacement_parentProduct childLaws
...
theorem deadlineMixedPrivateReplacement_evaluatedPayoff
  ...
  rw [deadlineMixedPrivateReplacement_parentMarginal]
  rfl
...
theorem expect_deadlineMixedPrivateClockLaw_payoff_eq_stoppingLaw
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
    (evaluation : WithTop ℕ → ℝ)
  ...
  rw [← deadlineMixedPrivateReplacement_evaluatedPayoff reward evaluation
...
/-- Ignoring the independent outsider deadline leaves the quiet child-product
baseline unchanged. -/
theorem map_cappedClockIndependentSample_quiet
    (childLaws : ι → PMF (Option ℕ)) (outsideLaw : PMF (Option ℕ)) :
    (cappedClockIndependentSample childLaws outsideLaw).map
        (fun sample => quietParentClocks sample.1) =
      pmfPi (quietParentStoppingLaws childLaws) := by
  rw [← map_pmfPi_child_quiet childLaws]
  unfold cappedClockIndependentSample
  rw [PMF.map_bind]
  have hmap (times : ι → Option ℕ) :
      (outsideLaw.map fun deadline => (times, deadline)).map
          (fun sample => quietParentClocks sample.1) =
        PMF.pure (quietParentClocks times) := by
    rw [PMF.map_comp]
    change outsideLaw.map (fun _ => quietParentClocks times) = _
    exact PMF.map_const outsideLaw (quietParentClocks times)
  simp_rw [hmap]
  exact PMF.bind_pure_comp _ _

omit [DecidableEq ι] in
/-- The coupled quiet baseline expectation is exactly the actual quiet
parent stopping-law payoff. -/
theorem expect_cappedClockIndependentSample_quiet_eq_stoppingLaw
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
    (evaluation : WithTop ℕ → ℝ)
    (childLaws : ι → PMF (Option ℕ)) (outsideLaw : PMF (Option ℕ))
    (who : Option ι) :
    _root_.Math.Probability.expect
      ...
      (fun sample => quittingPureClockEvaluatedPayoff reward evaluation
        (quietParentClocks sample.1) who) =
      quittingStoppingLawEvaluatedPayoff reward evaluation
        (quietParentStoppingLaws childLaws) who := by
  rw [quittingStoppingLawEvaluatedPayoff]
  rw [← map_cappedClockIndependentSample_quiet childLaws outsideLaw,
    _root_.Math.Probability.expect_map]
