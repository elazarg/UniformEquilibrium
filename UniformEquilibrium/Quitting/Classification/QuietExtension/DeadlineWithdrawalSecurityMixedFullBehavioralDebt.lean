import UniformEquilibrium.Quitting.Classification.QuietExtension.DeadlineWithdrawalSecurityMixedGain
import UniformEquilibrium.Quitting.Classification.QuietExtension.DeadlineWithdrawalSecurityMixedExpectation
...
/-!
# Full behavioral debt from security-enhanced deadline rows

The literal finite reward rows, actual reward-selected restart laws, exact
private product marginal, and unrestricted behavioral caps give the same
maximum-coefficient bound with the improved nonpositive singleton floor.
-/
...
local instance deadlineSecurityDebtChildNonempty :
...
theorem deadlineSecurity_outsideStoppingLawGain_le_weighted_behaviorDebt
  ...
  have hpoint := expect_deadlineSecurity_pointwise_le reward certificate
  ...
  have hmixed : (∑ i, expect law (fun sample =>
    ...
    intro i _
    exact (deadlineSecurityMixedPrivateReplacement_integratedGain reward evaluation
  ...
  exact hpoint.trans (Finset.sum_le_sum fun i _ =>
    mul_le_mul_of_nonneg_left
      (sub_le_sub_right
        (deadlineSecurityMixedChild_payoff_le_behaviorDeviationCap
...
theorem deadlineSecurity_outsideBehaviorDebt_le_weighted_childDebt
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
    (certificate : DeadlineSecurityRewardCertificate reward)
    (evaluation : WithTop ℕ → ℝ)
    (evaluation_nonneg : ∀ clock, 0 ≤ evaluation clock)
  ...
  simpa using
    (outsideBehaviorEvaluatedDeviationDebt_le_weighted_childDebt_add_const_of_stoppingLaw
      ...
      (fun outsideLaw => by simpa using
        (deadlineSecurity_outsideStoppingLawGain_le_weighted_behaviorDebt
...
theorem deadlineSecurity_quietLift_outsideBehaviorDebt_le_weighted_childDebt
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
    (certificate : DeadlineSecurityRewardCertificate reward)
    (evaluation : WithTop ℕ → ℝ)
    (evaluation_nonneg : ∀ clock, 0 ≤ evaluation clock)
  ...
  simpa using
    (quietLift_outsideBehaviorEvaluatedDeviationDebt_le_weighted_childDebt_add_const_of_canonical
      ...
      (by simpa using
        (deadlineSecurity_outsideBehaviorDebt_le_weighted_childDebt
...
theorem deadlineSecurity_quietLift_totalBehaviorDebt_le_weighted_childDebt
    (reward : {A : Finset (Option ι) // A.Nonempty} → Option ι → ℝ)
    (certificate : DeadlineSecurityRewardCertificate reward)
    (evaluation : WithTop ℕ → ℝ)
    (evaluation_nonneg : ∀ clock, 0 ≤ evaluation clock)
  ...
  simp_rw [hchild]
  have hout := deadlineSecurity_quietLift_outsideBehaviorDebt_le_weighted_childDebt
    reward certificate evaluation evaluation_nonneg evaluation_antitone
      childProfile
  dsimp only at hout
  calc
