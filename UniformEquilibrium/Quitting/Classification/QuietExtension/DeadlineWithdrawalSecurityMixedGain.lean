...
/-! # Exact integrated gain of the actual security mixture -/
...
theorem deadlineSecurityMixedPrivateReplacement_integratedGain
  ...
  have hpoint : expect law (fun sample =>
    ...
    funext sample
    exact deadlineSecurityMixedPrivateClockLaw_evaluatedGain_identity
  ...
  calc
    _ = max advanceWeight withdrawalWeight *
        (expect law mixed - expect law base) := by
      rw [expect_deadlineSecurityMixedPrivateClockLaw_payoff_eq_stoppingLaw
