...
def extendNormalSimplex (M : ι → ι → ℝ)
    (weight : Convexity.StdSimplex ℝ (normalCore M)) : Convexity.StdSimplex ℝ ι := by
  classical
  refine {
    weights := Finsupp.equivFunOnFinite.symm (extendNormalWeight M weight.weights)
    nonneg := ?_
    total := ?_ }
  · intro who
    by_cases hwho : who ∈ normalCore M
    · simp [extendNormalWeight, hwho, weight.weights_nonneg]
    · simp [extendNormalWeight, hwho]
  · rw [Finsupp.equivFunOnFinite_symm_sum]
    calc
      (∑ who, extendNormalWeight M weight.weights who) =
          ∑ who ∈ normalCore M, extendNormalWeight M weight.weights who := by
            symm
            apply Finset.sum_subset (Finset.subset_univ _)
            intro who _ hwho
            exact extendNormalWeight_of_notMem M weight.weights hwho
      _ = ∑ who : normalCore M, weight.weights who := by
        calc
          (∑ who ∈ normalCore M, extendNormalWeight M weight.weights who) =
              ∑ who : normalCore M,
                extendNormalWeight M weight.weights who.1 :=
            Finset.sum_subtype (normalCore M) (fun _ => Iff.rfl)
              (extendNormalWeight M weight.weights)
          _ = ∑ who : normalCore M, weight.weights who := by
            apply Finset.sum_congr rfl
            intro who _
            exact extendNormalWeight_of_mem M weight.weights who.property
      _ = 1 := weight.total_of_fintype

@[simp] theorem extendNormalSimplex_apply_of_mem
    (M : ι → ι → ℝ) (weight : Convexity.StdSimplex ℝ (normalCore M))
    {who : ι} (hwho : who ∈ normalCore M) :
    (extendNormalSimplex M weight).weights who = weight.weights ⟨who, hwho⟩ := by
  simp [extendNormalSimplex, hwho]

@[simp] theorem extendNormalSimplex_apply_of_notMem
    (M : ι → ι → ℝ) (weight : Convexity.StdSimplex ℝ (normalCore M))
    {who : ι} (hwho : who ∉ normalCore M) :
    (extendNormalSimplex M weight).weights who = 0 := by
  simp [extendNormalSimplex, hwho]
...
theorem singletonLCPResidual_extendNormalSimplex_of_mem
  ...
  calc
    (∑ owner,
        (extendNormalSimplex M weight).weights owner * M who.1 owner) =
        ∑ owner ∈ normalCore M,
          (extendNormalSimplex M weight).weights owner * M who.1 owner := by
      symm
      apply Finset.sum_subset (Finset.subset_univ _)
      intro owner _ howner
      rw [extendNormalSimplex_apply_of_notMem M weight howner, zero_mul]
    _ = ∑ owner : normalCore M,
        weight.weights owner * M who.1 owner.1 := by
      calc
        (∑ owner ∈ normalCore M,
            (extendNormalSimplex M weight).weights owner * M who.1 owner) =
            ∑ owner : normalCore M,
              (extendNormalSimplex M weight).weights owner.1 * M who.1 owner.1 :=
          Finset.sum_subtype (normalCore M) (fun _ => Iff.rfl)
            (fun owner => (extendNormalSimplex M weight).weights owner * M who.1 owner)
        _ = ∑ owner : normalCore M,
            weight.weights owner * M who.1 owner.1 := by
          apply Finset.sum_congr rfl
          intro owner _
...
theorem singletonLCPResidual_extendNormalSimplex_nonneg_of_notMem
  ...
  · exact mul_nonneg ((extendNormalSimplex M weight).weights_nonneg owner)
      (normalCore_entry_pos_of_notMem M hwho howner).le
  · change 0 ≤ (extendNormalSimplex M weight).weights owner * M who owner
    rw [extendNormalSimplex_apply_of_notMem M weight howner, zero_mul]
...
theorem fullHomogeneousWitness_singletonMixture
    ...
    (∀ who, reward (quittingSingletonTerminal who) who ≤
        quittingSingletonMixture reward weight.weights who) ∧
      (∀ who, 0 < weight.weights who →
        quittingSingletonMixture reward weight.weights who =
  ...
  have hbalance : ∀ who,
    ...
    change (∑ owner, weight.weights owner *
        reward (quittingSingletonTerminal owner) who) -
      (∑ owner, weight.weights owner) *
        reward (quittingSingletonTerminal who) who = _
    rw [weight.total_of_fintype, one_mul]
  constructor
  · intro who
    have := hresidual who
...
def homogeneousScaledRoot (weight : Convexity.StdSimplex ℝ ι)
    (scale : ℝ) (hscale0 : 0 ≤ scale) (hscale1 : scale ≤ 1) :
    ι → PMF Bool := fun who =>
  quittingHazardCoin (scale * weight.weights who)
    (mul_nonneg hscale0 (weight.weights_nonneg who)) (by
      have hweight : weight.weights who ≤ 1 := by
...
theorem quittingStationaryTotalHazard_homogeneousScaledRoot
  ...
  rw [← Finset.mul_sum, weight.total_of_fintype, mul_one]
...
theorem quittingStationarySingletonDirectionBarycenter_homogeneousScaledRoot
    ...
    quittingStationarySingletonDirectionBarycenter reward
      ...
      quittingSingletonMixture reward weight.weights who := by
...
theorem quittingStationaryTotalHazard_update_homogeneousScaledRoot
    ...
    quittingStationaryTotalHazard
        (Function.update (homogeneousScaledRoot weight scale hscale0 hscale1)
          who (PMF.pure false)) = scale * (1 - weight.weights who) := by
  classical
  unfold quittingStationaryTotalHazard
  ...
  have herase :
      ∑ owner ∈ Finset.univ.erase who,
        ...
        ∑ owner ∈ Finset.univ.erase who, scale * weight.weights owner := by
    apply Finset.sum_congr rfl
    intro owner howner
  ...
  rw [← Finset.mul_sum, weight.total_of_fintype]
  ring
...
theorem quittingStationarySingletonDirectionBarycenter_update_scaled_eq_mixture
    ...
    (weight : Convexity.StdSimplex ℝ ι) {scale : ℝ}
    (hscale0 : 0 ≤ scale) (hscale1 : scale ≤ 1) (hscale : scale ≠ 0)
    (who : ι) (hweight : weight.weights who < 1)
    (hpinned : weight.weights who = 0 ∨
      quittingSingletonMixture reward weight.weights who =
    ...
    quittingStationarySingletonDirectionBarycenter reward
      ...
      quittingSingletonMixture reward weight.weights who := by
  classical
  let mix := quittingSingletonMixture reward weight.weights who
  let solo := reward (quittingSingletonTerminal who) who
  have hden : scale * (1 - weight.weights who) ≠ 0 :=
    mul_ne_zero hscale (sub_ne_zero.mpr (ne_of_gt hweight))
  unfold quittingStationarySingletonDirectionBarycenter
  rw [quittingStationaryTotalHazard_update_homogeneousScaledRoot]
  have hsumErase :
      ∑ owner ∈ Finset.univ.erase who,
          weight.weights owner * quittingSoloReward reward owner who =
        mix - weight.weights who * solo := by
    have hsum := Finset.sum_erase_add
      (s := Finset.univ)
      (f := fun owner => weight.weights owner * quittingSoloReward reward owner who)
      (Finset.mem_univ who)
  ...
  calc
    (∑ owner,
        (((Function.update
          (homogeneousScaledRoot weight scale hscale0 hscale1) who
            (PMF.pure false)) owner true).toReal /
              (scale * (1 - weight.weights who))) *
          quittingSoloReward reward owner who) =
        (mix - weight.weights who * solo) / (1 - weight.weights who) := by
      rw [← Finset.sum_erase_add _ _ (Finset.mem_univ who)]
      rw [show
        (((Function.update
          (homogeneousScaledRoot weight scale hscale0 hscale1) who
      ...
      calc
        (∑ owner ∈ Finset.univ.erase who,
            scale * weight.weights owner / (scale * (1 - weight.weights who)) *
              quittingSoloReward reward owner who) =
            (∑ owner ∈ Finset.univ.erase who,
              weight.weights owner * quittingSoloReward reward owner who) /
                (1 - weight.weights who) := by
                  rw [Finset.sum_div]
                  apply Finset.sum_congr rfl
                  intro owner _
                  field_simp
        _ = (mix - weight.weights who * solo) / (1 - weight.weights who) := by
          rw [hsumErase]
        _ = (mix - weight.weights who * solo) / (1 - weight.weights who) := rfl
    _ = mix := by
...
theorem homogeneousScaledRoot_fixedOpponents_contracts
    ...
    (hnonvertex : ∀ who, weight.weights who < 1) :
...
theorem homogeneousScaledRoot_quitValue_le
    ...
    (hmixFloor : ∀ who,
      reward (quittingSingletonTerminal who) who ≤
        quittingSingletonMixture reward weight.weights who) (who : ι) :
    let root := homogeneousScaledRoot weight scale hscale.le
      (hscaleHalf.trans (by norm_num))
  ...
  calc
    ...
    _ ≤ quittingSingletonMixture reward weight.weights who + 2 * M * scale := by
      gcongr
      exact hmixFloor who
...
theorem homogeneousScaledRoot_neverValue_le
    ...
    (weight : Convexity.StdSimplex ℝ ι) {scale : ℝ}
    (hscale : 0 < scale) (hscaleHalf : scale ≤ 1 / 2)
    (hactivePinned : ∀ who, 0 < weight.weights who →
      quittingSingletonMixture reward weight.weights who =
        reward (quittingSingletonTerminal who) who)
    (hnonvertex : ∀ who, weight.weights who < 1) (who : ι) :
    let root := homogeneousScaledRoot weight scale hscale.le
      (hscaleHalf.trans (by norm_num))
  ...
  let M := quittingRewardBound reward
  have hM : 0 ≤ M := quittingRewardBound_nonneg reward
  have hweight0 := weight.weights_nonneg who
  ...
  have hhalf : quittingStationaryTotalHazard
    ...
    rw [htotal]
    calc
      scale * (1 - weight.weights who) ≤ scale * 1 := by
    ...
    quittingStationarySingletonDirectionBarycenter_update_scaled_eq_mixture
      reward weight hscale.le (hscaleHalf.trans (by norm_num)) hscale.ne'
        who (hnonvertex who) (by
          by_cases hz : weight.weights who = 0
          · exact Or.inl hz
  ...
  have herror : 6 * M * (scale * (1 - weight.weights who)) ≤ 6 * M * scale := by
    have hfactor : scale * (1 - weight.weights who) ≤ scale := by
      calc
        scale * (1 - weight.weights who) ≤ scale * 1 := by
          exact mul_le_mul_of_nonneg_left (by linarith) hscale.le
...
theorem isQuittingStationaryUniformEquilibriumPayoff_of_nonvertexHomogeneousWitness
    ...
    IsQuittingStationaryUniformEquilibriumPayoff reward
      (quittingSingletonMixture reward weight.weights) := by
...
theorem isUniformEquilibriumPayoff_singletonMixture_of_nonvertexHomogeneousWitness
    ...
    (quittingGame reward).IsUniformEquilibriumPayoff none
      (quittingSingletonMixture reward weight.weights) :=
...
theorem exists_uniformEquilibriumPayoff_of_nonvertexHomogeneousWitness
  ...
  exact ⟨quittingSingletonMixture reward weight.weights,
...
theorem singletonLCPResidual_eq_column_of_weight_eq_one
    ...
    {owner : ι} (howner : weight.weights owner = 1) (who : ι) :
    singletonLCPResidual M weight who = M who owner := by
  classical
  have hownerVal : weight.weights owner = 1 := howner
  have hzero : ∀ other, other ≠ owner → weight.weights other = 0 := by
    intro other hne
    have hsumErase : ∑ player ∈ Finset.univ.erase owner,
        weight.weights player = 0 := by
      have hsum := Finset.sum_erase_add (s := Finset.univ)
        (f := weight.weights) (Finset.mem_univ owner)
      rw [hownerVal, weight.total_of_fintype] at hsum
      linarith
    have hall := (Finset.sum_eq_zero_iff_of_nonneg
      (fun player _ => weight.weights_nonneg player)).mp hsumErase
  ...
  have herase : ∑ other ∈ Finset.univ.erase owner,
      weight.weights other * M who other = 0 := by
...
theorem exists_stationaryUniformEquilibriumPayoff_of_homogeneousMatrixBranch
  ...
  have hfullComplementary : ∀ who,
      fullWeight.weights who * singletonLCPResidual matrix fullWeight who = 0 := by
    intro who
    by_cases hwho : who ∈ normalCore matrix
    · rw [singletonLCPResidual_extendNormalSimplex_of_mem
        matrix weight ⟨who, hwho⟩,
