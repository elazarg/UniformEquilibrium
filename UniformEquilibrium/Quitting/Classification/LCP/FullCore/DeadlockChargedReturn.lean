...
The normalized full-core singleton comparison matrix studied here is
...
The four-block cap-clearance return has total-debt map
`D ↦ (2 / 9) * D + 1 / 18` and base clearance
`(3/4, 0, 1/4, 0)`. The base is semantically reachable from zero clearance:
...
/-- The displayed four-player full-core deadlock matrix. -/
...
/-- Every row has a distinct nonpositive comparison. -/
theorem deadlockMatrix_exists_ne_nonpos (who : Player) :
    ∃ owner, owner ≠ who ∧ deadlockMatrix who owner ≤ 0 := by
  fin_cases who
...
/-- Every player survives every layer of the normality recursion for the
deadlock matrix. -/
private theorem normalLayer_eq_univ_of_rows
    {ι : Type} [Fintype ι] [DecidableEq ι] {M : ι → ι → ℝ}
    (hrow : ∀ who, ∃ owner, owner ≠ who ∧ M who owner ≤ 0)
    (n : ℕ) : normalLayer M n = Finset.univ := by
  induction n with
  | zero => rfl
  | succ n ih =>
      rw [normalLayer, ih]
      refine Finset.filter_eq_self.mpr fun who _ => ?_
      obtain ⟨owner, hne, hentry⟩ := hrow who
      exact ⟨owner, Finset.mem_univ owner, hne, hentry⟩

theorem deadlockMatrix_normalLayer_eq_univ (n : ℕ) :
    normalLayer deadlockMatrix n = Finset.univ :=
  normalLayer_eq_univ_of_rows deadlockMatrix_exists_ne_nonpos n

/-- The deadlock matrix has full normal core. -/
theorem deadlockMatrix_normalCore_eq_univ :
    normalCore deadlockMatrix = Finset.univ := by
  ext who
  simp [normalCore, deadlockMatrix_normalLayer_eq_univ]

/-- A full-core deadlock completion is a four-player quitting reward table
whose normalized singleton comparison matrix is `deadlockMatrix`. Coalition
rewards involving two or more quitters remain unrestricted. -/
def IsFullCoreDeadlockCompletion
    (reward' : {S : Finset Player // S.Nonempty} → Payoff Player) : Prop :=
  normalizedSoloMatrix reward' = deadlockMatrix

theorem IsFullCoreDeadlockCompletion.normalCore_eq_univ
  ...
  rw [hcompletion]
  exact deadlockMatrix_normalCore_eq_univ
...
theorem reward_isFullCoreDeadlockCompletion :
    IsFullCoreDeadlockCompletion reward :=
  normalizedSoloMatrix_reward
...
theorem reward_globalDebtFloor_le_one_fourteenth
  ...
  exact globalDebtFloor_le_one_fourteenth reward
    (quittingNeverBoundarySemanticPair reward)
    reward_isFullCoreDeadlockCompletion never_capClearance never_mem_carrier
    δ hfloor
