...
projective Q-matrix, this module constructs a probability vector on that face
whose matrix residual is nonnegative and is zero in at least one coordinate.
This is the algebraic direction lemma used before a viability argument.
-/
...
private theorem projective_cemetery_lt_one
    ...
    (M : κ → κ → ℝ) (i0 : κ)
    (solution : ProjectiveLCPSolution M
  ...
  have hallzero : ∀ i, solution.singleton i = 0 :=
    sum_eq_zero_of_nonneg_of_add_eq_one solution.cemetery
      solution.singleton solution.singleton_nonneg solution.total heq
...
private theorem normalizeProjectiveSingleton_residual
  ...
  change (∑ owner, (solution.singleton owner /
      (1 - solution.cemetery)) * M who owner) = _
...
/-- Every nonempty projective-Q principal face of a zero-diagonal matrix has
a simplex direction with nonnegative residual on the face and at least one
tight residual coordinate. -/
  ...
  have hresFormula (who : players) :
      singletonLCPResidual (principalMatrix M players) weight who =
        (∑ owner, solution.singleton owner *
          principalMatrix M players who owner) /
            (1 - solution.cemetery) :=
