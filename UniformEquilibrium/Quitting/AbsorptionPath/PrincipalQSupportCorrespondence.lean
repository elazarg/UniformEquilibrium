...
def principalQSupportControls (q : ι → ℝ) : Set (Convexity.StdSimplex ℝ ι) :=
  {z | ∀ i, z.weights i * q i = 0}
...
theorem isCompact_principalQSupportControls (q : ι → ℝ) :
  ...
  have hcoordinate (i : ι) : IsClosed
      {z : Convexity.StdSimplex ℝ ι | z.weights i * q i = 0} :=
    isClosed_eq
      ((Convexity.StdSimplex.continuous_weights_apply ℝ i).mul continuous_const)
      continuous_const
  have hclosed : IsClosed (principalQSupportControls q) := by
    rw [show principalQSupportControls q =
        ⋂ i, {z | z.weights i * q i = 0} by
      ext z
      simp [principalQSupportControls]]
    exact isClosed_iInter hcoordinate
...
theorem upperHemicontinuous_principalQSupportControls :
  ...
  intro q u hopen hu
  let bad : Set ((ι → ℝ) × Convexity.StdSimplex ℝ ι) :=
...
def principalQVelocities (M : ι → ι → ℝ) (q : ι → ℝ) : Set (ι → ℝ) :=
  {velocity | ∃ z : Convexity.StdSimplex ℝ ι,
    z ∈ principalQSupportControls q ∧
...
theorem isCompact_principalQVelocities (M : ι → ι → ℝ) (q : ι → ℝ) :
  ...
  let velocity : Convexity.StdSimplex ℝ ι → (ι → ℝ) := fun z i =>
...
theorem convex_principalQVelocities (M : ι → ι → ℝ) (q : ι → ℝ) :
  ...
  let mixedValue : ι → ℝ := fun i =>
    a * z.weights i + b * w.weights i
  let mixed : Convexity.StdSimplex ℝ ι :=
    ⟨Finsupp.equivFunOnFinite.symm mixedValue,
      (fun i => by
        change 0 ≤ a * z.weights i + b * w.weights i
        exact add_nonneg (mul_nonneg ha (z.weights_nonneg i))
          (mul_nonneg hb (w.weights_nonneg i))),
      (by
        rw [Finsupp.equivFunOnFinite_symm_sum]
        simp only [mixedValue]
        rw [Finset.sum_add_distrib, ← Finset.mul_sum, ← Finset.mul_sum,
          z.total_of_fintype, w.total_of_fintype]
        simpa only [mul_one] using hab)⟩
  have hmixedSupport : mixed ∈ principalQSupportControls q := by
    intro i
    have hzi := hz i
    have hwi := hw i
    change (a * z.weights i + b * w.weights i) * q i = 0
  ...
  unfold singletonLCPResidual
  change a * ((∑ owner, z.weights owner * M i owner) - q i) +
      b * ((∑ owner, w.weights owner * M i owner) - q i) =
    (∑ owner, (a * z.weights owner + b * w.weights owner) * M i owner) - q i
  rw [show (∑ owner, (a * z.weights owner + b * w.weights owner) * M i owner) =
      a * ∑ owner, z.weights owner * M i owner +
        b * ∑ owner, w.weights owner * M i owner by
    rw [Finset.mul_sum, Finset.mul_sum, ← Finset.sum_add_distrib]
    apply Finset.sum_congr rfl
    intro owner _
    ring]
  calc
    a * ((∑ owner, z.weights owner * M i owner) - q i) +
        b * ((∑ owner, w.weights owner * M i owner) - q i) =
      a * ∑ owner, z.weights owner * M i owner +
        b * ∑ owner, w.weights owner * M i owner - (a + b) * q i := by ring
    _ = a * ∑ owner, z.weights owner * M i owner +
        b * ∑ owner, w.weights owner * M i owner - q i := by rw [hab, one_mul]
...
theorem abs_le_of_mem_principalQVelocities
  ...
  intro owner _
  rw [abs_mul]
  have hzabs : |z.weights owner| = z.weights owner :=
    abs_of_nonneg (z.weights_nonneg owner)
  rw [hzabs]
  exact mul_le_of_le_one_left (abs_nonneg _) (z.weights_apply_le_one owner)
