...
open Math.CyclicKofNArithmetic Math.CyclicKofNFiberLift
  Math.CyclicKofNPrimitiveBlocks CyclicKofNQuittingSchedule
...
/-- The full-orbit length `n` is a period of the schedule.  Distinct blocks
within one clock turn are recorded separately by
`exactCollapseSchedule_covers_translationPhase`. -/
...
/-- The full-orbit length `n` is a period of the behavioral root sequence.
For zero hazard the root sequence can have a smaller period. -/
...
theorem exactBellmanSpine_degree_exactCollapseBlock
    ...
    value time who = value (time + 1) who +
      ∑ degree ∈ Finset.range (k * d + 1),
        quittingActiveMobiusLayer reward (value (time + 1))
          ((exactCollapseSchedule n k d).active time)
          (roots time) who degree := by
  simpa only [card_exactCollapseBlock hk, exactCollapseSchedule] using
    exactBellmanSpine_value_eq_next_add_sum_layers_of_cyclicSchedule
