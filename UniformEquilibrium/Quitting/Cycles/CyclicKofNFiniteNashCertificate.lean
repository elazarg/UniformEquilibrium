...
nonempty translated block with positive hazard, including the sure-Quit
boundary, the finite Nash certificate alone produces a uniform-equilibrium
payoff. -/
theorem isUniformEquilibriumPayoff_of_finiteCyclicNashCertificate
    ...
    (hnash : IsFiniteCyclicNashCertificate reward A β hβpos.le hβ1)
    (initial : Fin (Fintype.card (TranslationPhase A))) :
    (quittingGame reward).IsUniformEquilibriumPayoff none
      (cyclicTerminalValues reward A β hβpos.le hβ1 initial) := by
  apply isUniformEquilibriumPayoff_quittingCyclicTerminalValue_of_certificate
    reward
    (cyclicPhaseRoots A β hβpos.le hβ1)
    (cyclicTerminalValues reward A β hβpos.le hβ1)
    initial
  · intro phase
    exact cyclicTerminalValues_eq_rootSuccessorPayoff
      reward A β hβpos.le hβ1 phase
  · exact hnash
  · intro who
    exact cyclicPhaseRoots_opponentContracts A hA hG β hβpos hβ1 who
