...
**Absorbing cyclic continuation block.**  `IsQuittingCyclicContinuationBlock
reward terminal (period + 1) block`.  This is the carrier itself.

**Zero deviation mismatch at every coordinate.**  This is supplied as an
explicit hypothesis and is *not* derived from the absorbing-block condition.
It is stated as
...
The admissibility hypothesis is not redundant.  The literal downstream
regression theorem `exists_cyclicContinuationBlock_not_cycleAdmissible` in
`Boundary/Repair/DisjunctionCounterexample.lean` exhibits an absorbing cyclic
continuation block whose cycle is not admissible.  It is stated downstream to
...
the row's quitter has a silent opponent).  The results below replace it by the
absorbing-block and cycle-admissibility hypotheses, which that example does
satisfy.
-/
...
/-- **An admissible absorbing cycle gives an exact terminal equilibrium.**
If `terminal` admits an
