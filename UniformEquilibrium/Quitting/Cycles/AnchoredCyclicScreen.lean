...
/-!
# The anchored cyclic response cap for single-quitter periodic schedules
...
exact on-path value is `quittingAnchoredCyclicOnPathValue`.  This file develops
the profile's exact finite best-response statistic and the max-linear response
...
At accuracy zero the response cap has a second reading.
...
an exactly Nash anchored cyclic profile into a bound of the response cap by
the on-path value.  That route consumes the profile's Nash property; it does
not compute the response cap.
...
The passage from a solution of that system to the evaluator's response cap
splits into two halves.  The deterministic-stop half is
`quittingAnchoredCyclicPhaseStop_le`.  The refusal half is discharged in
`UniformEquilibrium/Quitting/Cycles/AnchoredCyclicRenewal.lean`: the refusal
identity evaluates refusal against an anchored cyclic profile as the on-path
value of the same schedule with the refuser's own phases zeroed, which gives
...
`quittingAnchoredCyclicResponseCap_le_of_response_of_spectatorHazard`.
-/
