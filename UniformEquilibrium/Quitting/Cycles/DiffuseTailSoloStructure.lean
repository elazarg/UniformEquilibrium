...
An *exact diffuse tail* is a root sequence `roots` together with a value
sequence `value` and a boundary payoff satisfying an exact policy recursion
(`hpolicy`), exact endpoint Nash at accuracy zero (`hnash`), positive
remaining eventual absorption at every date (`hpositive`), and vanishing
conditioned mesh (`hmesh`), read through the conditioning of
...
* **Refusal degeneracy**
  (`quittingPeriodicWindowRefusalValue_eq_of_continueSurely`,
  `quittingPeriodicWindowRefusalValue_eq_zero_of_soloRoots`): a coordinate
  that already continues surely has a refusal value equal to its on-path
