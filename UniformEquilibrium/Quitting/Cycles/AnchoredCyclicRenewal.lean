...
/-!
# The anchored cyclic renewal system, and refusal against the profile

`quittingAnchoredCyclicOnPathValue_renewal`
(`UniformEquilibrium/Quitting/Cycles/AnchoredCyclicScreen.lean`) records one
step of the linear recursion `U^k = p_k * r({w k}) + (1 - p_k) * U^{k+1}`
around a period-`m` single-quitter cycle.  This module iterates that step and
solves the system, then identifies the refusal branch of the periodic
best-response statistic against the profile and discharges it against the
max-linear response system.
...
* Under `∏ k, (1 - p_k) < 1` the renewal weights, normalized by `1 - ∏ k,
  ...
  scheduled players (`quittingAnchoredCyclicOnPathValue_mem_convexHull`). Its
  contrapositive `quittingAnchoredCyclicOnPathValue_ne_of_not_mem_convexHull`
  is a prescreen on candidate targets, decided by the schedule's singleton
  geometry alone, before any best-response statistic is computed.

Deleting a player from an anchored cyclic cycle — forcing it to continue at
every phase — leaves the anchored cyclic cycle of the same schedule with the
...
never-absorbing branch alike.  On the degenerate branch — a refuser owning
every phase carrying positive hazard — the zeroed schedule never absorbs and
refusal is worth nothing
(`quittingPeriodicWindowRefusalValue_anchoredCyclic_eq_zero`).

The last section discharges the refusal comparison as a consequence of the
max-linear response system itself.  The anchored system is the single-quitter
case of the general periodic response system
(`isQuittingCyclicResponseSolution_of_isAnchoredCyclicResponseSolution`), and
the deviator's one-turn opponent survival along an anchored cyclic cycle is
...
`quittingAnchoredCyclicResponseCap_le_of_response_of_spectatorHazard`, which
bounds the exact behavioral best response outright.
-/
