...
`GameTheory.CyclicThreePlayerQuitting.AdmissibleCycle.lean`,
...
`not_isQuittingCyclicContinuationBlock_standardBlock_perturbedReward` shows that
`GameTheory.CyclicThreePlayerQuitting.AdmissibleCycle.standardBlock` -- the
*exact same* three-phase block data that is an admissible cyclic continuation
...
open GameTheory.CyclicThreePlayerQuitting.AdmissibleCycle
  (reward reward_singletonTerminal phaseRoot halfCoin halfCoin_true_toReal
    halfCoin_false_toReal phaseSimplexRoot phasePoint standardBlock
...
`GameTheory.CyclicThreePlayerQuitting.Minimality.nextThree`. -/
...
(`GameTheory.CyclicThreePlayerQuitting.AdmissibleCycle.reward`) plus a bonus `ε` at coordinate `who`
...
def perturbedReward (ε : ℝ) (quitters : {S : Finset Player // S.Nonempty}) :
    Payoff Player :=
  fun who =>
    reward quitters who + (if quitters.1 = {who, nextThree who} then ε else 0)

/-- At `ε = 0` the perturbed weight is literally the unperturbed one. -/
@[simp] theorem perturbedReward_zero : perturbedReward 0 = reward := by
  funext quitters who
  simp [perturbedReward]
...
theorem perturbedReward_singletonTerminal (ε : ℝ) (who : Player) :
...
value equals `1 > 0`. -/
theorem not_isQuittingZeroSolo_perturbedReward (ε : ℝ) :
    ¬ IsQuittingZeroSolo (perturbedReward ε) := by
  intro hzero
  have h := hzero 0
  rw [perturbedReward_singletonTerminal] at h
  norm_num at h
...
theorem perturbedReward_soloSelf (ε : ℝ) (i : Player) :
    quittingSoloReward (perturbedReward ε) i i = 1 :=
  perturbedReward_singletonTerminal ε i
...
theorem perturbedReward_collision_prevThree (ε : ℝ) (i : Player) :
    quittingSingletonCollisionReward (perturbedReward ε) i (prevThree i) = 1 + ε := by
  ...
  have hbase := reward_collision_prevThree i
...
theorem reward_soloOther_prevThree (i : Player) :
    quittingSoloReward reward i (prevThree i) = 0 := by
  fin_cases i <;> rfl

/-- The perturbation never touches a singleton coalition, so the previous
fact survives unchanged for the perturbed weight. -/
theorem perturbedReward_soloOther_prevThree (ε : ℝ) (i : Player) :
    quittingSoloReward (perturbedReward ε) i (prevThree i) = 0 := by
...
computation. -/
theorem not_quittingSoloQuitterCriterion_perturbedReward
    (ε : ℝ) (hε : 0 ≤ ε) (i : Player) (p : ℝ) (hp : 0 ≤ p) :
    ¬ QuittingSoloQuitterCriterion (perturbedReward ε) i p := by
  intro hcrit
  have hfail := hcrit (prevThree i) (prevThree_ne i)
  rw [perturbedReward_soloSelf, perturbedReward_collision_prevThree,
    perturbedReward_soloOther_prevThree] at hfail
...
`not_quittingSoloQuitterCriterion_perturbedReward`.
...
theorem not_isQuittingCyclicContinuationBlock_of_isolated
    ...
    ¬ IsQuittingCyclicContinuationBlock (perturbedReward ε) terminal 1 block := by
  intro hblock
  have hfixed := quittingCyclicContinuationBlock_one_fixed (perturbedReward ε)
  ...
  have hterminal : terminal = quittingSoloReward (perturbedReward ε) owner :=
    quittingCyclicContinuation_unique_of_absorbing_period_one (perturbedReward ε)
      terminal (quittingSoloReward (perturbedReward ε) owner)
      (quittingSoloStationaryRoot owner hazard) hfixed
      (quittingRootSuccessorPayoff_soloStationaryRoot_self (perturbedReward ε)
...
theorem quittingRootEndpointDifference_phaseRoot_eps (ε : ℝ) (c : Player) :
  ...
  rw [quittingRootQuitPayoff_soloStationaryRoot_other (perturbedReward ε) (prevThree_ne c),
    quittingRootContinuePayoff_soloStationaryRoot_other (perturbedReward ε) (prevThree_ne c),
    perturbedReward_soloSelf, perturbedReward_collision_prevThree,
    perturbedReward_soloOther_prevThree, standardPromise_value,
...
theorem not_isεQuittingRootEndpointNash_phaseRoot_eps
    (ε : ℝ) (hε : 0 < ε) (c : Player) :
    ¬ IsεQuittingRootEndpointNash (perturbedReward ε) (standardPromise (nextThree c)) 0
      (phaseRoot c) := by
  intro hnash
...
/-- **Deliverable 3.**
`GameTheory.CyclicThreePlayerQuitting.AdmissibleCycle.standardBlock` -- the
*exact same* three-phase block that is an admissible cyclic-continuation block
for the unperturbed table -- is **not** an
`IsQuittingCyclicContinuationBlock` for the `ε`-perturbed weight, at any
`ε > 0`. Only phase `0`'s edge is examined; this is a statement about *this
specific block*, not about every period-three block for this weight. -/
theorem not_isQuittingCyclicContinuationBlock_standardBlock_perturbedReward
    (ε : ℝ) (hε : 0 < ε) :
    ¬ IsQuittingCyclicContinuationBlock (perturbedReward ε) namedTarget 3 standardBlock := by
  intro hblock
  have hedge : IsQuittingNashBellmanEdge (perturbedReward ε) (phasePoint 0) (phasePoint 1) :=
...
end GameTheory.CyclicThreePlayerQuitting.PerturbedCycleExclusion
