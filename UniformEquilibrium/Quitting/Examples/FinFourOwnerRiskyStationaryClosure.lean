...
/-!
# A four-player owner-risky stationary closure

This file records an explicit owner-risky four-player quitting table.  Its
exact table algebra and preconditioner determinant are checked here.  The
whole-box diagonal estimate and the eight strict face inequalities are then
obtained from a centered mean-value bound: on the normalized unit box, dyadic
automatic differentiation of the reflected error polynomials supplies both
the value envelope at the center and the gradient row sums that control every
displacement from it.

The resulting interior stationary root is exact Nash against arbitrary
behavioral deviations.  This module does not construct or identify a maximal
cap ray and does not prove robustness under perturbations of the reward table.
-/
...
/-- The complete stationary-certificate type for the certified owner-risky
hazard box. -/
abbrev SharpStationaryCertificate (R singletonLevel : ℝ) :=
  QuittingConditionalFaceGapStationaryCertificate
    (sharpReward R singletonLevel)
    (quittingRationalBoxLower
      (quittingCenteredRationalBoxLower sharpCenter sharpHalfWidth))
    (quittingRationalBoxUpper
      (quittingCenteredRationalBoxUpper sharpCenter sharpHalfWidth))

/-- The rational lower endpoint of every certified hazard coordinate is
strictly positive. -/
theorem sharpBoxLower_pos (who : Player) :
    0 < quittingRationalBoxLower
      (quittingCenteredRationalBoxLower sharpCenter sharpHalfWidth) who := by
  fin_cases who <;>
    simp +decide [quittingRationalBoxLower, quittingCenteredRationalBoxLower,
      sharpCenter, sharpHalfWidth] <;> norm_num

/-- The rational upper endpoint of every certified hazard coordinate is
strictly below one. -/
theorem sharpBoxUpper_lt_one (who : Player) :
    quittingRationalBoxUpper
        (quittingCenteredRationalBoxUpper sharpCenter sharpHalfWidth) who < 1 := by
  fin_cases who <;>
    simp +decide [quittingRationalBoxUpper, quittingCenteredRationalBoxUpper,
      sharpCenter, sharpHalfWidth] <;> norm_num

/-- **Literal full-support stationary closure.**  For every certified parameter
there is an actual stationary certificate whose hazard lies in `(0,1)` at
every player.  Its stored fields expose the stationary profile, exact terminal
Nash theorem against arbitrary behavioral deviations, and uniform-equilibrium
payoff. -/
theorem exists_fullSupport_sharpStationaryCertificate
    (R singletonLevel : ℝ) (hR : R ∈ Icc 0 (1 / 37)) :
    ∃ certificate : SharpStationaryCertificate R singletonLevel,
      ∀ who, 0 < certificate.hazard who ∧ certificate.hazard who < 1 := by
  obtain ⟨certificate⟩ :=
    (sharpCenteredCertificate R singletonLevel hR).nonempty_stationaryCertificate
  refine ⟨certificate, ?_⟩
  intro who
  exact ⟨(sharpBoxLower_pos who).trans
      (certificate.lower_lt_hazard who),
    (certificate.hazard_le_upper who).trans_lt (sharpBoxUpper_lt_one who)⟩

/-!
The compiler above does not identify `sharpReward` with any maximal-ray reward
table.  Persistence under arbitrary nearby reward-table perturbations is also
a separate compact-uniformity theorem, not a consequence of parameter
uniformity inside this two-parameter family.
-/
