...
The enriched image is compact. Across consecutive exact edges, identity chronological transport
...
The positive coordinate co-state pairs exactly with the diagonal seam. Every
source seam is nonnegative; a supplied zero source can therefore expose its
zero face. Playerwise transport consequences belong to consumers that supply
such a source.
...
/-! ## Zero-source exposed faces -/

/-- If the carrier contains one flow with zero selected debt source, then the
negative selected-source co-state exposes exactly the flows whose selected
source vanishes. This is the precise attainment premise; no counterexample
or limiting argument is needed. -/
theorem mem_exposedFace_quittingDebtSourceZeroFaceCostate_iff_of_zero_mem
    (selected : ι)
    {zeroFlow : RawGradedFlow QuittingObstructionGrade
      (QuittingDebtSourceObstructionCoordinate ι)}
    (hzeroMem : zeroFlow ∈ quittingDebtSourceOneStageObstructionCarrier reward)
    (hzero : zeroFlow .charge (Sum.inr selected) = 0)
    (flow : RawGradedFlow QuittingObstructionGrade
      (QuittingDebtSourceObstructionCoordinate ι)) :
  ...
  · intro hface
    refine ⟨hface.1, ?_⟩
    have hmax := hface.2 zeroFlow hzeroMem
    rw [pair_quittingDebtSourceZeroFaceCostate,
      pair_quittingDebtSourceZeroFaceCostate, hzero, neg_zero] at hmax
    have hnonneg :=
      quittingDebtSourceObstructionCoordinate_nonneg hface.1 selected
    linarith
  · rintro ⟨hflow, hflowZero⟩
    refine ⟨hflow, ?_⟩
    intro candidate hcandidate
    rw [pair_quittingDebtSourceZeroFaceCostate,
      pair_quittingDebtSourceZeroFaceCostate, hflowZero, neg_zero]
    exact neg_nonpos.mpr
      (quittingDebtSourceObstructionCoordinate_nonneg hcandidate selected)

/-- With an attained zero selected source, zero-face membership of an exact
edge is equivalent to its playerwise augmented-cap transport equation. -/
theorem quittingDebtSourceObstructionFlow_mem_zeroFace_iff_cap_transport_apply_of_zero_mem
    (selected : ι)
    {zeroFlow : RawGradedFlow QuittingObstructionGrade
      (QuittingDebtSourceObstructionCoordinate ι)}
    (hzeroMem : zeroFlow ∈ quittingDebtSourceOneStageObstructionCarrier reward)
  ...
  rw [mem_exposedFace_quittingDebtSourceZeroFaceCostate_iff_of_zero_mem
    selected hzeroMem hzero]
  ...
  constructor
  · rintro ⟨_, hsourceZero⟩
    rw [quittingDebtSourceObstructionFlow_source] at hsourceZero
    unfold quittingDynamicDebtSeam at hsourceZero
    linarith
  · intro htransport
    refine ⟨hmem, ?_⟩
...
/-- A single attained flow with every debt-source coordinate zero makes
simultaneous zero-face membership equivalent to vector augmented-cap
transport. -/
theorem quittingDebtSourceObstructionFlow_mem_all_zeroFaces_iff_cap_transport_of_zero_mem
    {zeroFlow : RawGradedFlow QuittingObstructionGrade
      (QuittingDebtSourceObstructionCoordinate ι)}
    (hzeroMem : zeroFlow ∈ quittingDebtSourceOneStageObstructionCarrier reward)
    (hzero : ∀ selected, zeroFlow .charge (Sum.inr selected) = 0)
    (edge : QuittingDebtPoint ι × QuittingDebtPoint ι)
    (hedge : edge ∈ quittingFloorDynamicDebtEdgeGraph reward) :
    (∀ selected,
      quittingDebtSourceObstructionFlow edge ∈
  ...
  · intro hfaces
    ...
    exact
      (quittingDebtSourceObstructionFlow_mem_zeroFace_iff_cap_transport_apply_of_zero_mem
        selected hzeroMem (hzero selected) edge hedge).1 (hfaces selected)
  · intro htransport selected
    apply
      (quittingDebtSourceObstructionFlow_mem_zeroFace_iff_cap_transport_apply_of_zero_mem
        selected hzeroMem (hzero selected) edge hedge).2
