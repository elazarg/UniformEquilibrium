...
The quitting-game specialization of the scalar survival theorem in
`Math.Probability.SurvivalAmplification` uses the semantic edge laws to
produce positive bounded gaps and the one-step amplification inequality.
-/
