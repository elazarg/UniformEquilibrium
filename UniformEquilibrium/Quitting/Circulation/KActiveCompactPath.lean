...
The closed invariant is the vanishing of every squarefree monomial on more
than `K` coordinates.  For nonnegative hazard vectors this is equivalent to
having at most `K` positive coordinates, and compact chronological extraction
preserves it.
-/
...
realization serializes them.  Closed support extraction ensures that the
resulting infinite chronological PMF path has at most one positive
...
theorem exists_oneActiveSupportRationalDivergentPath_of_multiCirculation
  ...
  have hM : ∀ S j, |r S j| ≤ M := by
    intro S j
    simpa only [M, Real.norm_eq_abs] using
      (norm_le_pi_norm (r S) j).trans (norm_le_pi_norm r S)
  let s : ℕ := Fintype.card ι
  have hs : ∀ l, (mixSupport (C.mixWeight l)).card ≤ s := by
    intro l
    exact Finset.card_le_univ _
  let a : ℝ := Finset.univ.sup' Finset.univ_nonempty C.ratio
  have ha : ∀ l, C.ratio l ≤ a := by
    intro l
    exact Finset.le_sup' C.ratio (Finset.mem_univ l)
  have ha1 : a < 1 := by
    change Finset.univ.sup' Finset.univ_nonempty C.ratio < 1
    rw [Finset.sup'_lt_iff]
    exact fun l _ => C.ratio_lt_one l
...
theorem quittingGame_exists_uniformEquilibriumPayoff_of_KActivePaths
  ...
  letI : Nonempty ι := by
    ...
    obtain ⟨plan, _hsupport, hdiverges, _hir, _hK⟩ :=
      hpaths 1 (by norm_num)
    apply hdiverges
    have hzero : ∀ time, HasQuittingSupportCardAtMost 0 (plan time) := by
      intro time
      unfold HasQuittingSupportCardAtMost
      have hempty : quittingPositiveHazardSupport (plan time) = ∅ := by
        ext who
      ...
      simp [hempty]
    have hcharge : quittingTotalAbsorptionCharge plan = 0 := by
      funext time
      exact quittingTotalAbsorptionCharge_eq_zero_of_zeroActive
        plan hzero time
    rw [hcharge]
    exact summable_zero
  apply
