...
The perturbation formulas also give a depth-free mismatch penalty: the excess
over exact propagated debt is bounded first by a survival-weighted signed
penalty and then by the absolute boundary error.
-/
...
/-! ## Depth-free boundary-mismatch bounds -/
...
/-- Reinsertion exploitability at boundary mismatch `error` exceeds the exact
propagated debt by at most `(χ - P) * error⁺ + P * (-error)⁺`, independently of
the prefix depth. -/
...
/-- The depth-free mismatch penalty is at most the absolute mismatch. -/
