...
at a compatible packet, `ChargeTangent.SupportTransversality` proves that the
projective scale line is a nonzero radial-zero kernel and that derivative
surjectivity is incompatible with any positive-radial kernel direction.  A
projective gauge or radial-parameter formulation is therefore needed before
this interface can produce an arc.  No lifting, Puiseux resolution, or
arbitrary-player conclusion is asserted here.
-/
