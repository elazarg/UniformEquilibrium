...
theorem continuous_quittingRootOpponentAbsorptionMass_simplex
    {ι : Type} [Fintype ι] [DecidableEq ι] (owner : ι) :
...
/-- The robust Nash-defect moat specialized to a fixed owner's ordinary
...
theorem exists_eventually_totalNashDefect_moat_of_unique_allContinue_opponentAbsorption
  ...
  let highAbsorption : Set (QuittingRootSimplex ι) :=
    {root | eta ≤ quittingRootOpponentAbsorptionMass
      (quittingRootOfSimplex root) owner}
  ...
  · have hdefectContinuous : Continuous (fun root : QuittingRootSimplex ι =>
    ...
    have hselectedPositive : 0 < quittingRootTotalNashDefect reward cap
      ...
      have habsorptionZero : quittingRootOpponentAbsorptionMass
      ...
      change eta ≤ quittingRootOpponentAbsorptionMass
        (quittingRootOfSimplex selected) owner at hselectedHigh
    ...
    let bad : Set (Payoff ι × QuittingRootSimplex ι) :=
      {point | eta ≤ quittingRootOpponentAbsorptionMass
            (quittingRootOfSimplex point.2) owner ∧
        quittingRootTotalNashDefect reward point.1
            (quittingRootOfSimplex point.2) ≤ moat}
    have hbadClosed : IsClosed bad := by
      exact (isClosed_le continuous_const
        ((continuous_quittingRootOpponentAbsorptionMass_simplex owner).comp
