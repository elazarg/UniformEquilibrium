...
/-!
# Finite forced-owner defect-polarity dispatch

A forced-owner outsider defect on a terminal cylinder is charged either to
the actual live row or to a positive same-witness rectangle.  Actual-row
defect then splits by endpoint polarity: Continue-directed occupation is
collected by one behavioral deviation, while Quit-directed occupation
