...
/-- **The same source-matched packet is local on every frozen prefix.**
...
theorem exists_sourceMatchedChatteringWithUniformPrefixes
  ...
  · intro completed hcompleted partialCount hpartial observer
    exact abs_sum_chatteringPrefixCoefficient_mul_perturbed_column_le
      (fun mover observer ↦ frontier.tangent mover observer)
      (fun mover observer ↦ frontier.actualDebtDirection rank mover observer)
  ...
  · have hgainClose : ∀ mover,
      ...
      intro mover
      rw [actualGain, neg_sub_neg, abs_sub_comm]
    ...
    have hchargeBound :=
      abs_sum_chatteringCoefficient_mul_perturbed_charge_sub_one_le
        ...
        (fun mover ↦ frontier.actualGain rank mover) mass N hN hmass
        hcharge' hgainClose
