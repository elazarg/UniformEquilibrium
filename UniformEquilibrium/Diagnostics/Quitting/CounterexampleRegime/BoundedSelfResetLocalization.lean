...
/-! ## Regime-level two-way capstone -/

/-- The two sign-free endpoint geometries of bounded self-reset. -/
def HasQuittingBoundedSelfResetTwoWayLocalization
    {reward : {S : Finset ι // S.Nonempty} → Payoff ι}
    {regime : QuittingCounterexampleRegime reward}
    {starts : ℕ → (quittingGame reward).BehaviorProfile}
    (sequence : QuittingBoundedSelfResetTerminalSequence regime starts) : Prop :=
  Nonempty (QuittingBoundedSelfResetObserverAbsentWall sequence) ∨
    Nonempty (QuittingBoundedSelfResetTargetRowLocalization sequence)

theorem QuittingBoundedSelfResetTerminalSequence.twoWayLocalization
    {reward : {S : Finset ι // S.Nonempty} → Payoff ι}
    {regime : QuittingCounterexampleRegime reward}
    {starts : ℕ → (quittingGame reward).BehaviorProfile}
    (sequence : QuittingBoundedSelfResetTerminalSequence regime starts) :
    HasQuittingBoundedSelfResetTwoWayLocalization sequence := by
  by_cases hmem : sequence.observer ∈ sequence.terminal.val
  · exact Or.inr (sequence.targetRowLocalization hmem)
  · exact Or.inl (sequence.observerAbsentWall hmem)

/-- **Regime-level bounded self-reset localization.**  For every sequence of
starting profiles, a counterexample regime supplies a bounded executable
self-reset sequence entering exactly the observer-absent wall or the sign-free
target-row localization. -/
theorem QuittingCounterexampleRegime.exists_boundedSelfResetTwoWayLocalization
    {reward : {S : Finset ι // S.Nonempty} → Payoff ι}
    (regime : QuittingCounterexampleRegime reward)
    (starts : ℕ → (quittingGame reward).BehaviorProfile) :
    ∃ sequence : QuittingBoundedSelfResetTerminalSequence regime starts,
      HasQuittingBoundedSelfResetTwoWayLocalization sequence := by
  obtain ⟨sequence⟩ := regime.exists_boundedSelfResetTerminalSequence starts
  exact ⟨sequence, sequence.twoWayLocalization⟩

/-- Failure of ordinary uniform-equilibrium payoff existence produces a
counterexample regime and a two-way bounded self-reset localization from every
chosen sequence of starting profiles. -/
theorem exists_boundedSelfResetTwoWayLocalization_of_not_exists_uniformEquilibriumPayoff
    (reward : {S : Finset ι // S.Nonempty} → Payoff ι)
    (starts : ℕ → (quittingGame reward).BehaviorProfile)
    (hno : ¬ ∃ payoff : Payoff ι,
  ...
  obtain ⟨sequence, hlocalization⟩ :=
    regime.exists_boundedSelfResetTwoWayLocalization starts
  exact ⟨regime, sequence, hlocalization⟩
