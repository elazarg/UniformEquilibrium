...
* `isEmpty_terminalExploitabilityWitness_of_circulant_surplus_nonpos`
* `isEmpty_terminalExploitabilityWitness_of_circulant_surplus_eq_zero`
* `isEmpty_terminalExploitabilityWitness_of_pentagonCirculant_surplus_nonpos`
-/
...
regime. -/
theorem isEmpty_terminalExploitabilityWitness_of_circulant_surplus_nonpos
...
/-- A circulant table of vanishing surplus is in no terminal exploitability witness, by
the homogeneous branch. -/
theorem isEmpty_terminalExploitabilityWitness_of_circulant_surplus_eq_zero
    [Nontrivial ι] (h : HasCirculantSoloMatrix reward m)
    (hσ : (∑ d, m d) = 0) :
    IsEmpty (QuittingTerminalExploitabilityWitness reward) :=
  isEmpty_quittingTerminalExploitabilityWitness_of_exists_uniformEquilibriumPayoff _
...
theorem isEmpty_terminalExploitabilityWitness_of_pentagonCirculant_surplus_nonpos
