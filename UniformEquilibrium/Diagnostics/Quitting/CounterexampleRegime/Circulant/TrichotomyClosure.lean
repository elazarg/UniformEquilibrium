...
`QuittingTerminalExploitabilityWitness`.  This module reads each branch against the
...
terminal exploitability witness, or has positive margin sum with negative margins
forming exactly one complementary pair — and even that residual closes once
either of the two cap pairs of its pocket holds.

## Main results

* `isEmpty_terminalExploitabilityWitness_of_isFiringStep` — the firing-step branch
* `isEmpty_terminalExploitabilityWitness_of_nonneg_margins` — the solo exit branch
* `isEmpty_terminalExploitabilityWitness_of_unique_nonneg` — three negative margins
* `isEmpty_terminalExploitabilityWitness_of_neighbourPocket` and
  `isEmpty_terminalExploitabilityWitness_of_distantPocket` — either pocket at either cap
* `isEmpty_terminalExploitabilityWitness_of_nonpositiveSum_or_isFiringStep` — the two
  branches together
* `isEmpty_terminalExploitabilityWitness_of_not_isComplementaryPocketMargin` and
  `isEmpty_terminalExploitabilityWitness_or_isComplementaryPocketMargin` — the closure
  up to the complementary pockets, and the census they leave
* `isEmpty_terminalExploitabilityWitness_or_uncappedComplementaryPocket` — the same
...
/-- The firing-step branch, as emptiness of the terminal exploitability witness. -/
theorem isEmpty_terminalExploitabilityWitness_of_isFiringStep
...
/-- The solo exit branch, as emptiness of the terminal exploitability witness. -/
theorem isEmpty_terminalExploitabilityWitness_of_nonneg_margins
    (htable : IsCirculantPairTable reward s m J) (hs : 0 ≤ s)
...
/-- Three negative margins, as emptiness of the terminal exploitability witness. -/
theorem isEmpty_terminalExploitabilityWitness_of_unique_nonneg
    (htable : IsCirculantPairTable reward s m J) (hs : 0 ≤ s)
...
cap pairs holds. -/
theorem isEmpty_terminalExploitabilityWitness_of_neighbourPocket
    (htable : IsCirculantPairTable reward s m J) (hs : 0 ≤ s)
    (hjoin : ∀ d : ZMod 5, d ≠ 0 → J d ≤ 0)
  ...
  refine ⟨fun witness => witness.not_exists_uniformEquilibriumPayoff ?_⟩
...
theorem isEmpty_terminalExploitabilityWitness_of_distantPocket
    (htable : IsCirculantPairTable reward s m J) (hs : 0 ≤ s)
    (hjoin : ∀ d : ZMod 5, d ≠ 0 → J d ≤ 0)
...
solo self value and nonpositive join margins carries no terminal exploitability witness as
soon as its margin sum is nonpositive, or some step fires. -/
theorem isEmpty_terminalExploitabilityWitness_of_nonpositiveSum_or_isFiringStep
    (htable : IsCirculantPairTable reward s m J) (hs : 0 ≤ s)
    (hjoin : ∀ d : ZMod 5, d ≠ 0 → J d ≤ 0)
  ...
  · exact isEmpty_terminalExploitabilityWitness_of_pentagonCirculant_surplus_nonpos
      hcirculant hsum
  · by_cases hsum : (∑ e, m e) ≤ 0
    · exact isEmpty_terminalExploitabilityWitness_of_pentagonCirculant_surplus_nonpos
        hcirculant hsum
    · exact isEmpty_terminalExploitabilityWitness_of_isFiringStep htable hs (not_le.mp hsum)
...
terminal exploitability witness unless its margin sum is positive and its negative
...
theorem isEmpty_terminalExploitabilityWitness_of_not_isComplementaryPocketMargin
    (htable : IsCirculantPairTable reward s m J) (hs : 0 ≤ s)
    (hjoin : ∀ d : ZMod 5, d ≠ 0 → J d ≤ 0)
  ...
  · by_cases hneg : ∃ a : ZMod 5, a ≠ 0 ∧ m a < 0
    · refine isEmpty_terminalExploitabilityWitness_of_nonpositiveSum_or_isFiringStep htable
        hs hjoin ?_
      by_cases hsum : (∑ e, m e) ≤ 0
      · exact Or.inl hsum
      · exact Or.inr
          (exists_isFiringStep htable.margin_zero (not_le.mp hsum) hneg hpocket)
    · refine isEmpty_terminalExploitabilityWitness_of_nonneg_margins htable hs hjoin ?_
      intro d hd
...
value and nonpositive join margins either carries no terminal exploitability witness, or
...
theorem isEmpty_terminalExploitabilityWitness_or_isComplementaryPocketMargin
    (htable : IsCirculantPairTable reward s m J) (hs : 0 ≤ s)
    (hjoin : ∀ d : ZMod 5, d ≠ 0 → J d ≤ 0) :
    IsEmpty (QuittingTerminalExploitabilityWitness reward) ∨
  ...
  · by_cases hsum : (∑ e, m e) ≤ 0
    · exact Or.inl (isEmpty_terminalExploitabilityWitness_of_not_isComplementaryPocketMargin
...
nonnegative solo self value and nonpositive join margins either carries no
terminal exploitability witness, or has positive margin sum, negative margins forming
exactly one complementary pair, and *both* cap pairs of that pocket violated.

Against `isEmpty_terminalExploitabilityWitness_or_isComplementaryPocketMargin` this
...
theorem isEmpty_terminalExploitabilityWitness_or_uncappedComplementaryPocket
    (htable : IsCirculantPairTable reward s m J) (hs : 0 ≤ s)
    (hjoin : ∀ d : ZMod 5, d ≠ 0 → J d ≤ 0) :
    IsEmpty (QuittingTerminalExploitabilityWitness reward) ∨
      (0 < ∑ e, m e ∧
  ...
  rcases isEmpty_terminalExploitabilityWitness_or_isComplementaryPocketMargin htable hs
  ...
  · by_cases hfirst : J 1 ≤ m 1 ∧ J 2 ≤ m 1
    · exact Or.inl (isEmpty_terminalExploitabilityWitness_of_neighbourPocket htable hs hjoin
  ...
  · by_cases hfirst : J 1 ≤ m 3 ∧ J 3 ≤ m 3
    · exact Or.inl (isEmpty_terminalExploitabilityWitness_of_distantPocket htable hs hjoin
