...
/-!
# The collider completion, against the terminal exploitability witness
...
`QuittingTerminalExploitabilityWitness`.  This module reads the family against the
...
`isEmpty_quittingTerminalExploitabilityWitness_of_cappedJointExit` no longer carries an
...
* `isEmpty_terminalExploitabilityWitness_colliderFiringStep` — a firing step closes a
  collider completion of positive margin sum
* `isEmpty_terminalExploitabilityWitness_colliderThreeNegative` — three negative margins
  are closed with no further hypothesis
* `isEmpty_terminalExploitabilityWitness_colliderPocket` — a pocket collider table
  satisfying the floor carries no terminal exploitability witness
* `isEmpty_terminalExploitabilityWitness_colliderPocket_of_not_sureExit` — the same
  conclusion from failure of the sure-exit property at one adjacent pair
* `isEmpty_terminalExploitabilityWitness_colliderNeighbourFace` — a margin vector with
  `m 1 = 0` inside the equilibrium region and outside the `R₀` one
* `isEmpty_terminalExploitabilityWitness_colliderNeighbourPocket` — the neighbour pocket
  closes with no further hypothesis
* `isEmpty_terminalExploitabilityWitness_or_distantPocket` — the census: the distant
  pocket of positive margin sum is the whole residual
* `isEmpty_terminalExploitabilityWitness_colliderCompletion_closure` — the census read
  as a sufficient condition
* `isEmpty_terminalExploitabilityWitness_colliderUnitSolo` — the unconditional
...
larger than it carries no terminal exploitability witness as soon as some step fires. -/
theorem isEmpty_terminalExploitabilityWitness_colliderFiringStep
  ...
  isEmpty_terminalExploitabilityWitness_of_isFiringStep
...
is `g` carries no terminal exploitability witness, the witness being the constant-step
cyclic profile of step `4 * g`.  No screen on the larger coalitions and no
regime hypothesis enters. -/
theorem isEmpty_terminalExploitabilityWitness_colliderThreeNegative
    (hm0 : m 0 = 0) (hs : 0 ≤ s) (hls : low ≤ s) (hsum : 0 < ∑ e, m e)
    {g : ZMod 5} (hgm : 0 ≤ m g)
    (hother : ∀ e : ZMod 5, e ≠ 0 → e ≠ g → m e < 0) :
    IsEmpty (QuittingTerminalExploitabilityWitness (colliderReward s low m)) :=
  isEmpty_terminalExploitabilityWitness_of_unique_nonneg
...
theorem isEmpty_terminalExploitabilityWitness_colliderPocket
    (hm0 : m 0 = 0) (hs : 0 ≤ s)
  ...
  refine isEmpty_terminalExploitabilityWitness_constantStep
...
/-- **The collider-completion pocket carries no terminal exploitability witness.**  A
...
theorem isEmpty_terminalExploitabilityWitness_colliderPocket_of_not_sureExit
    (hm0 : m 0 = 0) (hs : 0 ≤ s) (hlow : low ≤ 0)
    (hm1 : m 1 ≤ 0) (hm4 : m 4 < 0) (hm2 : 0 ≤ m 2) (hm3 : 0 ≤ m 3)
    (hsum : 0 < m 1 + m 2 + m 3 + m 4) (y : ZMod 5)
    (hno : ¬ IsQuittingSureExitSet (colliderReward s low m) {y, y + 1}) :
    IsEmpty (QuittingTerminalExploitabilityWitness (colliderReward s low m)) :=
  isEmpty_terminalExploitabilityWitness_colliderPocket hm0 hs hm1 hm4 hm2 hm3 hsum
...
joint value no larger than it carries no terminal exploitability witness, even though its
...
theorem isEmpty_terminalExploitabilityWitness_colliderNeighbourFace (hs : 0 ≤ s)
    (hls : low ≤ s) :
    IsEmpty (QuittingTerminalExploitabilityWitness
      (colliderReward s low neighbourFaceMargin)) :=
  isEmpty_terminalExploitabilityWitness_colliderPocket neighbourFaceMargin_zero hs
...
/-- **The neighbour pocket carries no terminal exploitability witness.**  Either some
...
theorem isEmpty_terminalExploitabilityWitness_colliderNeighbourPocket
    (hm0 : m 0 = 0) (hs : 0 ≤ s) (hlow : low ≤ 0)
    (hm1 : m 1 ≤ 0) (hm4 : m 4 < 0) (hm2 : 0 ≤ m 2) (hm3 : 0 ≤ m 3)
    (hsum : 0 < m 1 + m 2 + m 3 + m 4) :
    IsEmpty (QuittingTerminalExploitabilityWitness (colliderReward s low m)) := by
  by_cases hsure : IsQuittingSureExitSet (colliderReward s low m) {0, 0 + 1}
  · exact isEmpty_quittingTerminalExploitabilityWitness_of_exists_uniformEquilibriumPayoff _
      ⟨_, isUniformEquilibriumPayoff_setReward_of_isQuittingSureExitSet _ hsure⟩
  · exact isEmpty_terminalExploitabilityWitness_colliderPocket_of_not_sureExit hm0 hs hlow
...
terminal exploitability witness, or has positive margin sum with its negative margins
exactly at the two distant distances.  That distant pocket is the whole
residual of the family. -/
theorem isEmpty_terminalExploitabilityWitness_or_distantPocket
    (hm0 : m 0 = 0) (hs : 0 ≤ s) (hlow : low ≤ 0) :
    IsEmpty (QuittingTerminalExploitabilityWitness (colliderReward s low m)) ∨
      (0 < m 1 + m 2 + m 3 + m 4 ∧ m 2 < 0 ∧ m 3 < 0 ∧ 0 ≤ m 4 ∧ 0 ≤ m 1) := by
  have hfive : (∑ e : ZMod 5, m e) = m 0 + m 1 + m 2 + m 3 + m 4 :=
    Fin.sum_univ_five (fun e : ZMod 5 => m e)
  rcases isEmpty_terminalExploitabilityWitness_or_isComplementaryPocketMargin
  ...
  · rw [hfive, hm0] at hsum
    ...
    · exact Or.inl (isEmpty_terminalExploitabilityWitness_colliderNeighbourPocket hm0 hs
...
carries no terminal exploitability witness as soon as its margin sum is nonpositive or
one of the two distant margins is nonnegative. -/
theorem isEmpty_terminalExploitabilityWitness_colliderCompletion_closure
    (hm0 : m 0 = 0) (hs : 0 ≤ s) (hlow : low ≤ 0)
...
`isEmpty_quittingTerminalExploitabilityWitness_of_cappedJointExit` as soon as `s = 1`
...
capped.  So the collider completion has no terminal exploitability witness whatever its
...
theorem isEmpty_terminalExploitabilityWitness_colliderUnitSolo
