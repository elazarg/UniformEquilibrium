...
/-!
# A terminal exploitability witness refutes the two profile-level existence branches
...
prescribed shape at every positive tolerance, so a terminal exploitability witness kills
...
/-- **A terminal exploitability witness refutes the stationary branch.**  Under a
...
theorem not_quittingStationaryεEquilibriumExistence
  ...
  intro hbranch
  obtain ⟨root, hroot⟩ :=
    hbranch (witness.terminalGap / 2) (by linarith [witness.terminalGap_pos])
  exact witness.not_isεAsymptoticNash_of_lt_terminalGap _
    (by linarith [witness.terminalGap_pos]) hroot

/-- **A terminal exploitability witness refutes the instant-punishment branch** for the
same reason. -/
