...
/-!
# Exclusions under a terminal exploitability witness at small player types
...
force every terminal exploitability witness to have at least four players.
-/
...
theorem elim_isEmpty [IsEmpty ι]
  ...
  obtain ⟨who, -, -⟩ :=
    witness.terminalExploitability fun i ↦ isEmptyElim i
  exact isEmptyElim who

/-- The regime is empty over any one-element player type. -/
theorem elim_unique [Unique ι]
    (witness : QuittingTerminalExploitabilityWitness reward) : False :=
...
theorem three_lt_card (witness : QuittingTerminalExploitabilityWitness reward) :
    3 < Fintype.card ι := by
  by_contra hle
  rw [not_lt] at hle
  have htwo := witness.one_lt_card
