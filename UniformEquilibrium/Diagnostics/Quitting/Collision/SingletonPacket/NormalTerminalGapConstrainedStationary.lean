...
theorem quittingFaceNumerator_update_self
  ...
  heterogeneousFaceNumerator_update_self r hazard who rate
...
theorem quittingFaceNumerator_congr_off_self
    ...
    quittingFaceNumerator r first who = quittingFaceNumerator r second who :=
  heterogeneousFaceNumerator_congr_off_self r first second who hagree

/-- The auxiliary compact game whose Nash roots are constrained stationary
best replies. -/
def normalTerminalGapConstrainedGame
    (reward : {S : Finset ι // S.Nonempty} → Payoff ι)
    (epsilon : ℝ) (hepsilon : epsilon ≤ 1) : CompactBarycentricGame :=
  heterogeneousStationaryFaceGame reward (fun _ => epsilon) (fun _ => hepsilon)
...
theorem exists_constrainedStationaryFaceNash
  ...
  simpa using exists_heterogeneousStationaryFaceNash
    reward (fun _ => epsilon) (fun _ => hepsilon)
...
theorem quittingRootAbsorptionMass_rootOfHazard_pos
  ...
  rootAbsorptionMass_rootOfHazard_pos_of_coordinate
    hazard hhazard0 hhazard1 hhazardPos
...
theorem quittingStationaryGain_rootOfHazard_eq_faceNumerator
  ...
  stationaryGain_rootOfHazard_eq_faceNumerator
