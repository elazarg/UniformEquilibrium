...
This is not an atlas source- or trace-preservation result.  If another
construction supplies the pair `base`, only that pair and the same reward table
are reused; its minimum-law source, low-tail row, monodromy trace, and edge
certificates are not fields of this dispatch.
-/
...
/-- The dispatch records that its prescribed base has exactly two players. -/
theorem base_card
    (dispatch : FinFourPairBasePaidCapSemanticDispatch reward witness base) :
    base.card = 2 :=
  dispatch.localization.base_card
