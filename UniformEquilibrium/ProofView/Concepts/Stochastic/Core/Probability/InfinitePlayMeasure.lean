...
* `StochasticGame.infinitePlayMeasureFromStagePMF` — the same construction from an arbitrary
  distribution over the first state-action outcome
...
* `StochasticGame.map_frestrictLe_infinitePlayMeasureFromStagePMF` — the corresponding finite
  coordinate projection for an arbitrary first-outcome law
...
/-- The infinite-play measure generated by an arbitrary distribution of the stage-`0`
state-action outcome and the behavioral profile's subsequent transition kernels. -/
def infinitePlayMeasureFromStagePMF
    (σ : G.BehaviorProfile) (initial : PMF G.StageOutcome) : Measure G.Play :=
  Kernel.trajMeasure (X := fun _ : ℕ => G.StageOutcome) initial.toMeasure
    (G.stepKernel σ)

instance instIsProbabilityMeasureInfinitePlayMeasureFromStagePMF
    (σ : G.BehaviorProfile) (initial : PMF G.StageOutcome) :
    IsProbabilityMeasure (G.infinitePlayMeasureFromStagePMF σ initial) := by
  unfold infinitePlayMeasureFromStagePMF
  infer_instance
...
def infinitePlayMeasure (σ : G.BehaviorProfile) (s₀ : G.State) : Measure G.Play :=
  G.infinitePlayMeasureFromStagePMF σ (G.initialPMF σ s₀)
...
/-- Finite coordinate laws generated from an arbitrary stage-`0` outcome distribution. -/
def coordsDistFromStagePMF
    (σ : G.BehaviorProfile) (initial : PMF G.StageOutcome) :
    (t : ℕ) → PMF (∀ _ : Finset.Iic t, G.StageOutcome) :=
  fun t => Nat.rec (initial.map fun z (_ : Finset.Iic 0) => z)
    (fun n ih => ih.bind fun x => (G.stepPMF σ n x).map (G.extendCoords x)) t

omit [DecidableEq ι] [Countable G.State] [∀ i, Countable (G.Act i)] in
/-- The arbitrary-initial coordinate law obeys the same one-step recursion as `coordsDist`. -/
theorem coordsDistFromStagePMF_succ
    (σ : G.BehaviorProfile) (initial : PMF G.StageOutcome) (t : ℕ) :
    G.coordsDistFromStagePMF σ initial (t + 1) =
      (G.coordsDistFromStagePMF σ initial t).bind fun x =>
        (G.stepPMF σ t x).map (G.extendCoords x) :=
  rfl
...
def coordsDist (σ : G.BehaviorProfile) (s₀ : G.State) :
    (t : ℕ) → PMF (∀ _ : Finset.Iic t, G.StageOutcome) :=
  G.coordsDistFromStagePMF σ (G.initialPMF σ s₀)
...
/-- Transport an arbitrary stage-`0` outcome law to the singleton coordinate family used by
`Kernel.trajMeasure`. -/
def startMeasureFromStagePMF (initial : PMF G.StageOutcome) :
    Measure (∀ _ : Finset.Iic 0, G.StageOutcome) :=
  initial.toMeasure.map (MeasurableEquiv.piUnique
    (fun _ : Finset.Iic 0 => G.StageOutcome)).symm

omit [Fintype ι] [DecidableEq ι] [Countable G.State] [∀ i, Countable (G.Act i)] in
/-- The transported arbitrary initial measure is the measure of its singleton-coordinate
PMF image. -/
theorem startMeasureFromStagePMF_eq (initial : PMF G.StageOutcome) :
    G.startMeasureFromStagePMF initial =
      initial.toMeasure.map fun z (_ : Finset.Iic 0) => z := by
  have hfun : ⇑(MeasurableEquiv.piUnique (fun _ : Finset.Iic 0 => G.StageOutcome)).symm =
    ...
    subst hi
    rfl
  rw [startMeasureFromStagePMF, hfun]

/-- The singleton-coordinate starting measure for the canonical initial state. -/
def startMeasure (σ : G.BehaviorProfile) (s₀ : G.State) :
    Measure (∀ _ : Finset.Iic 0, G.StageOutcome) :=
  G.startMeasureFromStagePMF (G.initialPMF σ s₀)

omit [DecidableEq ι] [Countable G.State] [∀ i, Countable (G.Act i)] in
/-- `startMeasure` is `initialMeasure` transported along the singleton-coordinate gluing
map, matching `coordsDist`'s stage-`0` distribution. -/
theorem startMeasure_eq (σ : G.BehaviorProfile) (s₀ : G.State) :
  ...
  simpa [startMeasure, initialMeasure] using
    G.startMeasureFromStagePMF_eq (G.initialPMF σ s₀)

omit [DecidableEq ι] in
/-- The projective-family identity for an arbitrary stage-`0` outcome distribution. -/
theorem bind_partialTraj_eq_coordsDistFromStagePMF_toMeasure
    (σ : G.BehaviorProfile) (initial : PMF G.StageOutcome) (t : ℕ) :
    Measure.bind (G.startMeasureFromStagePMF initial)
      (partialTraj (X := fun _ : ℕ => G.StageOutcome) (G.stepKernel σ) 0 t) =
      (G.coordsDistFromStagePMF σ initial t).toMeasure := by
  induction t with
  | zero =>
    rw [partialTraj_self]
    ...
    rw [this, Measure.bind_dirac, G.startMeasureFromStagePMF_eq,
      PMF.toMeasure_map (p := initial) (hf := Measurable.of_discrete)]
    rfl
  | succ t ih =>
    have heq : partialTraj (X := fun _ : ℕ => G.StageOutcome)
        (G.stepKernel σ) 0 (t + 1) =
        partialTraj (X := fun _ : ℕ => G.StageOutcome)
            (G.stepKernel σ) t (t + 1) ∘ₖ
          partialTraj (X := fun _ : ℕ => G.StageOutcome) (G.stepKernel σ) 0 t :=
      partialTraj_succ_eq_comp (Nat.zero_le t)
    have hcomp : (⇑(partialTraj (X := fun _ : ℕ => G.StageOutcome)
        (G.stepKernel σ) t (t + 1) ∘ₖ
          partialTraj (X := fun _ : ℕ => G.StageOutcome) (G.stepKernel σ) 0 t)) =
        fun x => Measure.bind
          (partialTraj (X := fun _ : ℕ => G.StageOutcome) (G.stepKernel σ) 0 t x)
          (partialTraj (X := fun _ : ℕ => G.StageOutcome)
            (G.stepKernel σ) t (t + 1)) :=
    ...
    have hfun : (⇑(partialTraj (X := fun _ : ℕ => G.StageOutcome)
        (G.stepKernel σ) t (t + 1))) =
        fun x => ((G.stepPMF σ t x).map (G.extendCoords x)).toMeasure :=
      funext (G.stepKernel_partialTraj_succ_self σ t)
    rw [hfun, measureBind_toMeasure_eq, G.coordsDistFromStagePMF_succ]
...
theorem bind_partialTraj_eq_coordsDist_toMeasure (σ : G.BehaviorProfile) (s₀ : G.State)
  ...
  exact G.bind_partialTraj_eq_coordsDistFromStagePMF_toMeasure
    σ (G.initialPMF σ s₀) t
...
omit [DecidableEq ι] in
/-- Restricting an arbitrary-initial infinite play to its first `t + 1` coordinates gives
the corresponding finite recursive coordinate law. -/
theorem map_frestrictLe_infinitePlayMeasureFromStagePMF
    (σ : G.BehaviorProfile) (initial : PMF G.StageOutcome) (t : ℕ) :
    (G.infinitePlayMeasureFromStagePMF σ initial).map
        (Preorder.frestrictLe (π := fun _ : ℕ => G.StageOutcome) t) =
      (G.coordsDistFromStagePMF σ initial t).toMeasure := by
  have hbind : G.infinitePlayMeasureFromStagePMF σ initial =
      Measure.bind (G.startMeasureFromStagePMF initial)
  ...
  rw [hpt, G.bind_partialTraj_eq_coordsDistFromStagePMF_toMeasure]

omit [DecidableEq ι] in
/-- Restricting the canonical infinite play to its first `t + 1` state-action coordinates
gives the finite coordinate law computed by `coordsDist`. -/
theorem map_frestrictLe_infinitePlayMeasure
    (σ : G.BehaviorProfile) (s₀ : G.State) (t : ℕ) :
    (G.infinitePlayMeasure σ s₀).map
        (Preorder.frestrictLe (π := fun _ : ℕ => G.StageOutcome) t) =
      (G.coordsDist σ s₀ t).toMeasure := by
  exact G.map_frestrictLe_infinitePlayMeasureFromStagePMF
    σ (G.initialPMF σ s₀) t
