...
| Simon Question 1 boundary | `MathUE/Topology/ExtendedOrbit.lean`, `MathUE/Topology/CompactEdgeBudgetedPrefixRelation.lean`, `MathUE/Topology/SimonViabilityQuestion.lean`, `MathUE/Topology/SimonViabilityBudgetCompiler.lean`, `UniformEquilibrium/Quitting/Classification/AbnormalPlayers.lean` | The generic strict bounded-potential theorem `not_hasArbitrarilyLargeFiniteOrbitVariationWith_of_potential_bounds` rules out arbitrarily large finite-orbit variation, but no Simon `F_epsilon` certificate, necessity bridge, or game-semantic conclusion is supplied. Independently, a positive restartable extension yields one compatible path satisfying a linearly diverging prefix budget and hence `QuestionOneConclusion`; the adapter `QuestionOneHypotheses.conclusion_of_restartableEscape` is conditional on that extra restartability input. Simon's seven hypotheses do not currently derive restartability, so `QuestionOneAffirmative` remains open. The abnormal-player layer separately supplies the attained positive minimum abnormality gap. |
