...
| Quitting counterexample localization | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeFiveWayLocalization.lean` | The exact equivalence between nonexistence and a bundled counterexample regime carrying its exhaustive five-way stopping-law localization. Singleton compression is an ambient handoff, not a source-matched chronology. |
| Positive-collision reached rows | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeReachedRowDebtLocalization.lean` | The canonical positive-collision branch yields a strict subsequence and an explicit uniform positive lower bound for one fixed non-observer's legal reached-row gain; nonconvergence is a projection. This does not provide source re-entry or an exact return. |
