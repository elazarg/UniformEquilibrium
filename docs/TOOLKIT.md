...
| Source-matched chronological boundary | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegime/StoppingLaw/SourceMatchedChronologicalData.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegime/StoppingLaw/SourceMatchedChronologicalSurvival.lean` | Source-matched radial reset profiles produce executable scheduled roots and their exact chronological data. A repeated finite sample has vanishing joint and deleted-player survival under explicit one-period contraction. The checked regressions show that a debt-decreasing frozen reset need not create opponent exposure and that exact zero forcing need not make the initial debt small. The missing block/splice producer must still obtain contraction and non-tautological candidate tails from the reset-cube packet. |
