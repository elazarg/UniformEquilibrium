...
| Source-matched stopping-law chattering | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegime/StoppingLaw/SourceMatchedResetCube.lean` | A flat charged circulation yields finite actual common-source packets, uniform `O(1/N)` frozen-prefix control, and a literal reset-cube edge adapter. Signed square curvature is routed to the simultaneous-reset orientation consumer; rotor or reprojection compilation is available only conditionally on the surviving near-return branch. No chronological carrier path is produced. |
