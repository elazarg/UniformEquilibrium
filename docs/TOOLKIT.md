...
| Aligned collision and preemption | `UniformEquilibrium/Quitting/Boundary/Repair/AlignedPreemptionCollision.lean` | In the sequential two-solo screen, the follower's exact immediate-Quit gain is the owner's rate times the collision gain; the preemption inequality does not enter. For simultaneous sure-blocker repair, the exact three-condition characterization reduces to owner optimality and spectator no-join whenever one displayed scalar punishment-floor inequality makes blocker balance automatic. |
