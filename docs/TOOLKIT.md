...
| Survival-weighted suffix regret | `UniformEquilibrium/Quitting/Paths/SurvivalWeightedSuffixRegret.lean` | Conditional source-to-suffix pure-time regret transport and its `sSup` cap inequality, plus a quitting-game adapter to four-corner witness switching. The prefix transport and chronological renewal inputs remain producers to be supplied. |
