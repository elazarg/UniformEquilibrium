...
| Solan--Vieille solo-hazard finite ledger | `UniformEquilibrium/Quitting/Examples/SolanVieilleBoundarySoloHazardLedger.lean` | `boundarySoloBudgetIdentity` and its finite-prefix consequences check the exact solo-hazard friction ledger, unabsorbed-mass estimate, and finite owner-mass floor under the stated prefix hypotheses. The infinite semantic reduction, exact arbitrary-schedule pair-death argument, uniform positive exploitability floor, and rational upper certificate remain unproved; this file is only the finite algebraic ledger. |
