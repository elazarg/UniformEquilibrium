...
| Exact charged-packet amplification | `MathUE/ChargedPacketAmplification.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegime/ChargedPacketAmplification.lean` | `exists_path_charge_gt_of_uniform_tube_packet` concatenates genuine source-matched finite relation paths available at every reachable tube state into paths of arbitrarily large charge. `QuittingCounterexampleRegime.false_of_uniform_reachable_packet_producer` then contradicts the canonical finite prefix-charge capacity. This is the final consumer of an exact packet producer: it does not lift a stopping-law tangent to a predecessor path, identify residual gain with relation charge, or prove persistent packet availability after the source changes. |
| Full admissible-cycle amplification | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegime/Capacity/AdmissibleCycle.lean` | `quittingGame_exists_uniformPayoff_of_positive_admissible_cycle` iterates any positive closed path in the full punishment-floor admissible exact Nash--Bellman relation, decodes arbitrarily charged literal finite prefixes, and obtains a uniform-equilibrium payoff. `quittingGame_exists_uniformPayoff_of_positive_admissible_return` reduces this to one positive exact edge plus an exact admissible return. Conversely, `QuittingCounterexampleRegime.admissible_cycle_chargeSum_eq_zero` makes every full admissible cycle zero-charge in a counterexample regime. These are exact return consumers; they do not construct an edge or return from stopping-law geometry. |
...
| Finite linear charged capacity | `MathUE/FiniteLinearChargedCapacity.lean` | `normalizedPositiveChargedCirculation_iff_unboundedFeasibleCharge` is the exact recession theorem for flat finite charged columns. In the alternative arm, `exists_strictWeight_capacityBound_of_noCirculation` supplies a strictly positive Lyapunov weight and a finite bound for every orthant-feasible mass. The module also proves one-column boundary hitting, flat total-debt preservation, quantitative weighted descent, strict lower-potential co-debtor extraction, and boundary support of positive-charge maximizers. These are numerical linear statements, not semantic realization or executable chronology. |
...
| Exact-diagonal stopping-law endpoints | `UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticStoppingLawVanishingRegretTangentExtraction.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegime/StoppingLaw/OffDiagonal/PotentialCoDecreaseEndpoint.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegime/StoppingLaw/OffDiagonal/PotentialCoDecreaseCurvature.lean` | Vanishing-regret replacement selection upgrades every maintained counterexample frontier to exact tangent diagonal and zero limiting full-replacement mover debt. In the flat potential arm, `potentialCoDecrease_minimumFiberDeflation_or_paidFirstDisagreement` gives an exact endpoint dichotomy: on the minimum-debt fiber the active support/cardinality and a companion debt strictly decrease; off that fiber a fixed positive exact paid first-disagreement row occurs eventually along the same source/reset subsequence. The potential arm requires at least three active debtors; at rank at most two, flat no-entry data force charged circulation. Repetition still requires re-extraction at the new minimum, and the paid row has no chronological return consumer here. |
...
| Quitting counterexample localization | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegime/BoundedSelfResetLocalization.lean` | For every starting-profile sequence in a counterexample regime, a pure-time self-reset chain of length at most `card(I)+1` yields either an observer-absent forced-owner wall or an observer-containing reached-row gain. The certificate records a fixed terminal and explicit positive mass/gain floors. No converse is claimed from the branch data. |
