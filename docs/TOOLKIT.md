...
| Quitting counterexample localization | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegime/StoppingLaw/ThreeWayLocalization.lean` | Nonexistence produces one counterexample regime, an exhaustive stopping-law frontier, and a self-oriented pure-time atom routed into its three-way localization. The prescribed-payoff comparison is absent; no converse is claimed from the branch data. |
