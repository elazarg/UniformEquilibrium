...
| Quitting counterexample localization | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegime/StoppingLaw/FourWayLocalization.lean` | Nonexistence produces one counterexample regime, an exhaustive stopping-law frontier for that regime, and its four-way localization. Observer-containing singleton packets enter the signed actual-row branches without a cardinality premise. No converse is claimed from the branch data. |
