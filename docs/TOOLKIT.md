...
| Balanced singleton cycles | `UniformEquilibrium/Quitting/Cycles/BalancedSingletonCertificate.lean` | A one-owner-at-a-time cyclic Bellman certificate with owner indifference, passive-player solo floors, and one positive opponent hazard per player compiles to a uniform-equilibrium payoff. Finite mesh and collision caps are inferred automatically, so nonsingleton coalition rewards remain arbitrary. |
