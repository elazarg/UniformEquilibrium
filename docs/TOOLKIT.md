...
| Punishment-floor payoff near-returns | `UniformEquilibrium/Quitting/Projective/PunishmentFloorNearReturn.lean`, `UniformEquilibrium/Diagnostics/Quitting/PaidFirstDisagreementPayoffNearReturn.lean` | `quittingGame_exists_uniformEquilibriumPayoff_of_admissiblePath_payoffNearReturns` consumes one fixed positive charge threshold while source, target, path, and charged edge may vary with endpoint tolerance and endpoint payoff vectors converge arbitrarily closely. `PaidFirstDisagreementAdmissiblePayoffNearReturnConsumer` and `exists_uniformEquilibriumPayoff_of_finiteSupportRankExitPayoffNearReturnConsumers` expose this primary target. Fixed-edge payoff closure and exact return are stronger special cases; none of these producers is constructed here. |
...
| Source-matched chronological boundary | `UniformEquilibrium/Diagnostics/Quitting/Chronology/SourceMatchedChronologicalData.lean`, `UniformEquilibrium/Diagnostics/Quitting/Chronology/SourceMatchedRepeatedRootSurvival.lean`, `UniformEquilibrium/Diagnostics/Quitting/Regression/SourceMatchedExposureNoGo.lean`, `UniformEquilibrium/Diagnostics/Quitting/Frozen/RadialPacketExposure.lean` | Source-matched radial reset profiles produce executable scheduled roots and their exact chronological data. A repeated finite sample has vanishing joint and deleted-player survival under explicit one-period contraction. Independently, a positive balanced circulation produces a fresh simultaneous radial packet with two distinct positive marginals and one finite cutoff exposing the joint and every one-player-deleted clock by a fixed coefficient times the genuine frontier scale. This packet-local exposure is not persistent residual chronology. The checked regression shows that a debt-decreasing frozen reset need not create opponent exposure and that exact zero forcing need not make the initial debt small. The missing block/splice producer must still obtain contraction and non-tautological candidate tails from the reset-cube packet. |
...
| Simon finite-horizon variation account | `MathUE/Probability/HarmonicStateAccount.lean` | `finiteExpectedSpaceTimeMarkovVariation_le_card` gives Simon's finite-state cardinality bound from a supplied `HasStatewiseMarkovVariationBudget` escape/Poisson certificate. The space-time centering and finite variation accounting are checked, but Simon Lemma 2 remains open: the escape/Poisson certificate and the infinite adapter from return probabilities are not produced. Raw absorption charge in the quitting packet/cycle route is a different quantity from Euclidean orbit variation and does not feed this obstruction automatically. |
...
| Simon survival-crossing repair | `UniformEquilibrium/Quitting/Classification/SimonFiniteOrbit/SurvivalCrossingRepair.lean` | `quittingPunishmentFloorClipAt` dominates the actual continuation payoff, and `exists_quittingContinuation_deviation_ge_floorClip` supplies an actual unilateral deviation whose payoff is at least the clipped floor (the clip itself contains the explicit slack `η`); it does not claim that a best-response supremum is attained. `floorClippedPurificationCertificate_of_spliceNash` supplies the local purified row directly to `quittingJointSurvivalWeight_firstCrossing_interval`, which checks the strict first-crossing interval. The coordinate-closeness and survival conclusions remain conditional on supplied endpoint modulus, normalized near-feasibility, no-sure-quitter, and corrected uniform-survival predicates (with an explicit positive `ρ`); no global conditioning/prefix extraction or finite orbit is produced. |
| Simon compact alternatives | `UniformEquilibrium/Quitting/Classification/SimonFiniteOrbit/CompactQuantitativeAlternatives.lean` | `quittingInstantPunishmentεEquilibriumExistence_of_nearTotalSupportRows` turns uniformly bounded rational support rows with absorption tending to one into the instant-punishment branch, using an explicit sure-Quit rounding bound. The normalized-motion/escape producer and a common positive `ρ` remain supplied; this is not Simon Lemma 2 or the full finite-orbit theorem. |
...
| Projective Q-bar principal decoder | `UniformEquilibrium/Quitting/Classification/LCP/ProjectiveQBarBehavioralDecoder.lean` and its adapters under `UniformEquilibrium/Quitting/AbsorptionPath/` | `exists_punishmentNormal_singletonPath_of_projectiveQBar` supplies the continuous path on the punishment-normal principal matrix; `ContinuousZeroPerfectSingletonPath.ambientLift` checks the ambient omitted-player/minmax lift, while `UniformEquilibrium/Quitting/AbsorptionPath/SingletonPathRates.lean` supplies ordinary-clock rates and active pinning, and the decoder consumer supplies the arbitrary-behavior pure-time compiler. `exists_uniformEquilibriumPayoff_of_punishmentNormal_projectiveQBar_of_pathDecoder` and `exists_normal_nonprojectivePrincipal_of_pathDecoder_of_counterexample` are conditional on `QuittingPunishmentNormalPathDecoder`: logarithmic-rate/integral reconstruction, the deleted-clock Snell identity and exceptional-owner split, uniform product-law discretization, and conversion to actual terminal profiles remain the central unproved decoder. The restriction is only to the principal matrix of punishment-normal players, stronger than full-matrix Q-bar. |
...
| Returned-block homogeneous tangent obstruction | `UniformEquilibrium/Quitting/Stationary/ReturnedBlockTangentObstruction.lean` and `UniformEquilibrium/Quitting/Classification/LCP/ReturnedBlockTangentGap.lean` | `hasHomogeneousSimplexSolution_of_vanishing_returnedBlocks` consumes arbitrary-varying-horizon returned product blocks whose total hazard vanishes and whose aggregate Bellman and probability-weighted endpoint regrets are little-o of that hazard. `relativeError_gap_of_noHomogeneous` strengthens the converse to an explicit `R0`-margin scale and relative-error gap. Principal-reward and recursive-normal-core adapters are checked for literal restricted blocks. No block, chronology, or unrestricted-behavior strategy is produced. |
