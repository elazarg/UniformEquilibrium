...
| Source-matched chronological boundary | `UniformEquilibrium/Quitting/Debt/Dynamic/NashBellmanChronologicalForcing.lean`, `UniformEquilibrium/Quitting/Paths/PersistentDeletedClockTwoLabel.lean`, `UniformEquilibrium/Diagnostics/Quitting/Chronology/SourceMatchedChronologicalData.lean`, and `UniformEquilibrium/Diagnostics/Quitting/Frozen/RadialPacketExposure.lean` | Exact Nash--Bellman spines make every prescribed-discrepancy, adverse-forcing, generated-secant, and initial-debt field identically zero. Persistent joint and every one-player-deleted survival are exactly reduced to two fixed labels with divergent marginal hazard; summable reprojection loss or fixed-fraction retention preserves the conclusion. The complete remaining exact-spine producer is therefore to select one exact spine whose same roots preserve two distinct persistent labels. A fresh frozen packet with two positive marginals does not by itself provide that persistence or source compatibility. |
