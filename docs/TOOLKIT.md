...
| Conditional face-gap stationary class | `UniformEquilibrium/Quitting/Stationary/FaceNumerator.lean`, `UniformEquilibrium/Quitting/Classification/Existence/ConditionalFaceGap.lean` | The division-free face numerator is continuous on the full hazard cube and equals the opponent-absorption factor times the conditional face gap away from the zero denominator. On an arbitrary coordinatewise box, strict lower-face and weak upper-face signs (with no derangement assumption) yield a common zero; the exact behavioral terminal Nash and uniform-equilibrium capstone then follow. Strict upper-face signs additionally force a fully interior zero. This is a special source-data class, not a universal producer; no range/Bernstein or five-player adapter is claimed. |
