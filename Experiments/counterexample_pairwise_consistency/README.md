...
The master self-contained formulation is
`questions/Question172-BoundedBellmanChargeVersusTerminalInstability.md`.
That launch packet is not an implementation dependency of these Experiments.
...
| `P × W` | Consistent | [`PW_PACKET_WINDOW_REPORT.md`](PW_PACKET_WINDOW_REPORT.md), `PWPacketWindowConsistency.lean` | Exact occupation-linked regression: packet refusal predicts the stabilized window refusal branch; `delivery → target` is false. |
